functions_mass_spectrometry <- function() {
  ################## FUNCTIONS - MASS SPECTROMETRY 2018.07.17 ################
  ...
  check_internet_connection <<-
    function(method = "getURL",
             website_to_ping = "www.google.it") {
      ##### Start with getURL...
        ...
        try({
          ...
          if ("RCurl" %in% installed.packages()[, 1]) {
            ...
            install.packages(
              ...
              destdir = Sys.getenv("R_LIBS_USER")
            )
            library(RCurl)
          }
        }, silent = TRUE)
      ...
      if (method == "ping" || there_is_internet == FALSE) {
        if (Sys.info()[1] == "Linux") {
          ...
          there_is_internet <-
            !as.logical(
              system(
                command = paste("ping -c 1 -W 2", website_to_ping),
                intern = FALSE,
                ignore.stdout = TRUE,
                ignore.stderr = TRUE
              )
          ...
          there_is_internet <-
            !as.logical(
              system(
                command = paste("ping -n 1 -w 2000", website_to_ping),
                intern = FALSE,
                ignore.stdout = TRUE,
                ignore.stderr = TRUE
              )
          ...
          there_is_internet <-
            !as.logical(
              system(
                command = paste("ping", website_to_ping),
                intern = FALSE,
                ignore.stdout = TRUE,
                ignore.stderr = TRUE
              )
  ...
  install_and_load_required_packages <<-
    function(required_packages,
             repository = "http://cran.mirror.garr.it/mirrors/CRAN/",
      ...
      if ((length(missing_packages) > 0 &&
           there_is_internet == TRUE) || length(missing_packages) == 0) {
  ...
  ensemble_vote_classification <<-
    function(classification_matrix,
             class_list = NULL,
             weighted_decision_method = "bayesian probabilities",
             performance_parameter_list = NULL,
             type_of_validation_for_performance_estimation = "cv",
             alpha = 0.05) {
      ### Class list
      ...
      if (is.null(class_list) || length(class_list) == 0) {
        ...
        for (cl in 1:ncol(classification_matrix)) {
          class_vector <-
            append(class_vector, as.character(classification_matrix[, cl]))
        }
        ...
        if (all(!is.na(as.character(original_classification_matrix[, i])))) {
          ...
          if (is.null(classification_matrix)) {
            classification_matrix <-
              as.matrix(cbind(as.character(
                original_classification_matrix[, i]
              )))
          } else {
            classification_matrix <-
              cbind(classification_matrix, as.matrix(cbind(
                as.character(original_classification_matrix[, i])
              )))
          }
          # Retain thr column names
          final_colnames <-
            append(final_colnames,
                   colnames(original_classification_matrix)[i])
        } else {
          # Defines the models which are to be discarded from the ensemble predictions
          na_models <- append(na_models, i)
        }
      }
      rownames(classification_matrix) <-
        rownames(original_classification_matrix)
      ...
      if (weighted_decision_method == "unweighted majority") {
        ...
        classification_ensemble_matrix <-
          cbind(
            apply(
              X = classification_matrix,
              MARGIN = 1,
              FUN = function(x)
                majority_vote_function(x, class_list)
            )
          )
        colnames(classification_ensemble_matrix) <-
          "Ensemble classification"
      } else if (weighted_decision_method == "bayesian probabilities" &&
                 (
                   !is.null(performance_parameter_list) &&
                   is.list(performance_parameter_list) &&
                   length(performance_parameter_list) > 0
                 )) {
        ##### Majority vote: bayesian probabilities
        # Determine the type of validation (cross or external) to use
        if (type_of_validation_for_performance_estimation == "cv" ||
            type_of_validation_for_performance_estimation == "cross validation" ||
            type_of_validation_for_performance_estimation == "CV") {
          model_performance_parameter_list <- performance_parameter_list$cv
        } else if (type_of_validation_for_performance_estimation == "ev" ||
                   type_of_validation_for_performance_estimation == "external validation" ||
                   type_of_validation_for_performance_estimation == "EV") {
          model_performance_parameter_list <-
            performance_parameter_list$external
        }
        ...
        bayesian_ensemble_predictions_subfunction <- function(x) {
          ...
          for (m in 1:length(model_performance_parameter_list)) {
            ...
            for (cl in 1:length(class_list)) {
              ...
              if (predicted_classes_models[m] == class_list[cl]) {
                bayesian_prob <-
                  model_performance_parameter_list[[m]][[(predicted_classes_models[m])]][["TPR"]]
              } else {
                bayesian_prob <-
                  model_performance_parameter_list[[m]][[(predicted_classes_models[m])]][["FPR"]]
              }
              ...
              if (is.null(class_probs_list[[(class_list[cl])]])) {
                ...
                class_probs_list[[(class_list[cl])]] <-
                  append(class_probs_list[[(class_list[cl])]], bayesian_prob)
              }
            }
          }
          ...
          for (l in 1:length(class_probs_list)) {
            class_probs_list[[l]] <-
              append(class_probs_list[[l]],
                     model_performance_parameter_list[[1]][[(names(class_probs_list)[l])]][["class proportion"]])
          }
          # Calculate the product of probabilities
          final_class_probs <- list()
          for (l in 1:length(class_probs_list)) {
            final_class_probs[[(names(class_probs_list)[l])]] <-
              prod(class_probs_list[[l]], na.rm = TRUE)
          }
          # Extract the most probable class
          final_class_vector <- unlist(final_class_probs)
          most_probable_class <-
            as.matrix(names(which(
              final_class_vector == max(final_class_vector, na.rm = TRUE)
            )))
          # Return
        ...
        classification_ensemble_matrix <-
          as.matrix(cbind(
            apply(
              X = as.matrix(classification_matrix),
              MARGIN = 1,
              FUN = bayesian_ensemble_predictions_subfunction
            )
          ))
        colnames(classification_ensemble_matrix) <-
          "Ensemble classification"
  ...
  matrix_add_class_and_sample <<-
    function(signal_matrix,
      ...
      if (isMassPeaksList(peaks)) {
        for (i in 1:length(peaks)) {
          if (spectra_format == "imzML" ||
              spectra_format == "txt" ||
              spectra_format == "csv" || spectra_format == "msd") {
            path_vector <- append(path_vector, peaks[[i]]@metaData$file[1])
          } else if (spectra_format == "fid") {
            path_vector <-
              append(path_vector, peaks[[i]]@metaData$sampleName[1])
          }
        }
      } else if (isMassPeaks(peaks)) {
        if (spectra_format == "imzML" ||
      ...
      peaks <-
        replace_sample_name(peaks, spectra_format = spectra_format)
      # Create the empty vector
      ...
      if (isMassPeaksList(peaks)) {
        for (i in 1:length(peaks)) {
          if (spectra_format == "imzML" ||
            ...
            file_vector <-
              append(file_vector, peaks[[i]]@metaData$sampleName[1])
          }
        }
      } else if (isMassPeaks(peaks)) {
        if (spectra_format == "imzML" ||
      ...
      ################################## Only the sample
      if ((class_output == FALSE &&
           sample_output == TRUE) ||
          (class_output == TRUE &&
           length(class_list) == 0 && sample_output == TRUE)) {
        ...
        sample_column <-
          matrix("", ncol = 1, nrow = number_of_spectra)
      ...
      if (class_output == TRUE &&
          length(class_list) >= 1 && sample_output == TRUE) {
        # Create the sample matrix column and append it to the global matrix
        # Sample as rownames
        if ("sample" %in% row_labels || "Sample" %in% row_labels) {
          rownames(signal_matrix) <- file_vector
      ...
      ################################## Only the class
      if (class_output == TRUE &&
          length(class_list) >= 1 && sample_output == FALSE) {
        class_list <- sort(class_list)
        # Rename the classes according to the class_list vector (the match should be /class/ to avoid catching the name of the class in previous folders)
        class_vector <- path_vector
        for (p in 1:length(class_vector)) {
    ...
    function(class_list = c("HP", "PTC"),
      ...
      if (length(class_list) > length(outcome_list)) {
        ...
        for (i in 1:abs(length(outcome_list) - length(class_list))) {
          class_list <-
            append(class_list, outcome_list[length(class_list) + i])
        }
      }
      ### Generate the matrix (classes)
      class_outcome_matrix <-
        matrix("", nrow = greater_number, ncol = 4)
      # Define the colnames
      colnames(class_outcome_matrix) <-
        c("Class", "Outcome", "Number", "Color")
      # Fill the matrix
      ...
      return(
        list(
          class_outcome_matrix = class_outcome_matrix,
          class_vector_as_numeric = class_vector,
          legend_text = class_list,
          legend_fill = outcome_list_as_color
        )
      )
    }
  ...
  matrix_add_thy <<-
    function(signal_matrix, peaks, spectra_format = "imzML") {
      number_of_spectra <- length(peaks)
      # Create the empty vector
      file_vector <- character()
      # Add the file names recursively, scrolling the whole spectral dataset
  ...
  remove_low_intensity_peaks <<-
    function(peaks,
             low_intensity_peak_removal_threshold_percent = 1,
      ...
      if (low_intensity_peak_removal_threshold_percent > 0 &&
        ...
        if (low_intensity_peak_removal_threshold_method == "element-wise" ||
            low_intensity_peak_removal_threshold_method == "spectrum-wise") {
          ##### INTENSITY FILTERING FUNCTION (ELEMENT-WISE)
          intensity_filtering_subfunction_element <-
            function(peaks,
                     low_intensity_peak_removal_threshold_percent) {
              ...
              values_to_be_discarded <-
                intensity_values[((
                  intensity_values * 100 / max(intensity_values, na.rm = TRUE)
                ) < low_intensity_peak_removal_threshold_percent
                )]
              ...
              if (length(values_to_be_discarded) > 0) {
                ...
                for (i in 1:length(values_to_be_discarded)) {
                  value_position <-
                    which(intensity_values == values_to_be_discarded[i])
                  positions_to_be_discarded <-
                    append(positions_to_be_discarded, value_position)
                }
                # Discard the values from the vectors
                intensity_values <-
                  intensity_values [-positions_to_be_discarded]
          ...
          if (isMassPeaksList(peaks)) {
            ### MULTICORE
            if ((is.logical(allow_parallelization) &&
                 allow_parallelization == TRUE) ||
                (is.character(allow_parallelization) &&
            ...
            } else if (is.character(allow_parallelization) &&
              ...
              peaks_filtered <- list()
              peaks_filtered <-
                foreach(i = 1:length(peaks),
                        .packages = "MALDIquant") %dopar% {
                          peaks_filtered[[i]] <-
                            intensity_filtering_subfunction_element(peaks[[i]],
                                                                    low_intensity_peak_removal_threshold_percent = low_intensity_peak_removal_threshold_percent)
                        }
            ...
            peaks_filtered <-
              intensity_filtering_subfunction_element(peaks,
                                                      low_intensity_peak_removal_threshold_percent)
          }
        }
        ########## WHOLE DATASET
        if (low_intensity_peak_removal_threshold_method == "whole") {
          ##### INTENSITY FILTERING FUNCTION (ELEMENT-WISE)
          intensity_filtering_subfunction_whole <-
            function(peaks,
                     low_intensity_peak_removal_threshold_percent,
                     highest_intensity) {
              # Filter out the peaks whose intensity is below a certain threshold
              ...
              # Identify the positions of the values to be discarded
              values_to_be_discarded <-
                intensity_values[((intensity_values * 100 / highest_intensity) < low_intensity_peak_removal_threshold_percent
                )]
              # If there are values to be discarded...
              if (length(values_to_be_discarded) > 0) {
                # Identify the positions
            ...
            if (length(peaks[[p]]@mass) > 0) {
              for (m in 1:length(peaks[[p]]@mass)) {
                if (highest_intensity == 0 ||
                    peaks[[p]]@intensity[m] > highest_intensity) {
          ...
          if (isMassPeaksList(peaks)) {
            ...
            ########## Single peaks element
            peaks_filtered <-
              intensity_filtering_subfunction_whole(peaks,
                                                    low_intensity_peak_removal_threshold_percent,
                                                    highest_intensity)
          }
        }
  ...
  outliers_removal <<- function (v, replace_with = "") {
    ...
    iqr_fences <-
      c((summary_vector[2] - 1.5 * inter_quartile_range),
        (summary_vector[5] + 1.5 * inter_quartile_range)
      )
    # Find the outliers based on the fences condition
    outliers_position <-
      which(v < iqr_fences[1] | v > iqr_fences[2])
  ...
  peak_statistics <<-
    function(spectra,
             ...
             peak_filtering_frequency_threshold_percent = 25,
             remove_outliers = TRUE,
             low_intensity_peak_removal_threshold_percent = 0,
             low_intensity_peak_removal_threshold_method = "element_wise",
             tolerance_ppm = NULL) {
      ########## Load the required libraries
      require(XML)
      require(MALDIquant)
      ...
      if (length(class_list) == 0 ||
          length(class_list) == 1 || is.null(class_list)) {
        number_of_classes <- 1
      ...
      peaks <-
        align_and_filter_peaks(
          ...
          peak_filtering_frequency_threshold_percent = peak_filtering_frequency_threshold_percent,
          low_intensity_peak_removal_threshold_percent = low_intensity_peak_removal_threshold_percent,
      ...
      if (number_of_classes == 1) {
        ...
        peak_statistcs_function <<-
          function (signal_matrix_column,
                    signal_matrix,
                    remove_outliers) {
            # Generate the output matrix row
            peak_stat_matrix_row <- matrix (0, nrow = 1, ncol = 7)
            rownames(peak_stat_matrix_row) <-
              as.numeric(colnames(signal_matrix_column))
            colnames(peak_stat_matrix_row) <-
              c(
                "Intensity distribution type",
                "Mean",
                "Standard deviation",
            ...
            if (length(intensity_vector) >= 3 &
              ...
              distribution_type <-
                "Number of samples too high, assume it is normal"
            }
            # Other parameters
            ...
            peak_stat_matrix_row [, 5] <-
              as.numeric(median_intensityensity)
            peak_stat_matrix_row [, 6] <-
              as.numeric(inter_quartile_range)
            peak_stat_matrix_row [, 7] <-
              paste("1st quartile",
                    first_quartile,
                    "; 3rd quartile",
                    third_quartile)
        ...
        # Output matrix
        peak_stat_matrix <-
          matrix (0, nrow = (ncol(signal_matrix) - 1), ncol = 8)
        rownames(peak_stat_matrix) <-
          as.numeric(colnames(signal_matrix)[1:(ncol(signal_matrix) - 1)])
        colnames(peak_stat_matrix) <-
          c(
            "Intensity distribution type",
            "Mean",
            "Standard deviation",
        ...
        # Only peaks
        signal_matrix_peaks <-
          signal_matrix [, 1:(ncol(signal_matrix) - 1)]
        ...
        peak_stat_matrix <-
          t(
            apply(
              ...
              MARGIN = 2,
              FUN = function(x)
                peak_statistcs_function(x, signal_matrix, remove_outliers = remove_outliers)
            )
          )
        ...
        for (pk in 1:ncol(intensity_matrix_with_na)) {
          ...
          spectra_counter_vector <-
            append(spectra_counter_vector, length(intensity_vector[!is.na(intensity_vector)]))
        }
        peak_stat_matrix <-
          cbind(peak_stat_matrix, spectra_counter_vector)
        ...
        for (i in 1:length(class_list)) {
          observations_per_class <-
            append(observations_per_class, length(which(signal_matrix[, ncol(signal_matrix)] == class_list[i])))
        }
        ...
        if (sufficient_number_of_observations_per_class == TRUE) {
          # Output matrix
          peak_stat_matrix <-
            matrix (0, nrow = (ncol(signal_matrix) - 2), ncol = 14)
          rownames(peak_stat_matrix) <-
            as.numeric(colnames(signal_matrix)[1:(ncol(signal_matrix) - 2)])
          colnames(peak_stat_matrix) <-
            c(
              "Intensity distribution type",
              "Mean",
              ...
              "Homoscedasticity (non-parametric)",
              "t-Test",
              "ANOVA",
          ...
          for (p in 1:(ncol(signal_matrix) - 2)) {
            ...
            for (l in 1:length(class_list)) {
              ...
              intensity_vector[[l]] <-
                as.numeric(signal_matrix [signal_matrix[, ncol(signal_matrix)] == class_list[l], p])
            }
            if (remove_outliers == TRUE) {
            ...
            for (l in 1:length(class_list)) {
              if (length(intensity_vector[[l]]) >= 3 &&
                  length(intensity_vector[[l]]) <= 5000) {
              ...
              if (length(intensity_vector[[l]]) < 3) {
                distribution_type[[l]] <-
                  "Not determinable, number of samples too low"
              }
            ...
            if (length(class_list) == 2) {
              variance_test_parametric <-
                var.test(intensity_vector[[1]], intensity_vector[[2]])
            }
            if (length(class_list) >= 2) {
              variance_test_non_parametric <-
                bartlett.test(as.numeric(signal_matrix[, p]), g = as.factor(signal_matrix[, ncol(signal_matrix)]))
            }
            ...
            for (l in 1:length(class_list)) {
              ...
              summary_intensity_vector [[l]] <-
                summary(intensity_vector[[l]])
              mean_intensity[[l]] <- summary_intensity_vector[[l]] [4]
              coeff_variation[[l]] <-
                (st_dev_intensity[[l]] / mean_intensity[[l]]) * 100
              median_intensityensity[[l]] <-
                summary_intensity_vector[[l]] [3]
              first_quartile[[l]] <- summary_intensity_vector[[l]] [2]
              third_quartile[[l]] <- summary_intensity_vector[[l]] [5]
              inter_quartile_range[[l]] <-
                third_quartile[[l]] - first_quartile[[l]]
            ...
            if (length(class_list) >= 2) {
              anova_test <-
                aov(signal_matrix[, p] ~ signal_matrix[, ncol(signal_matrix)])
            }
            # WILCOXON - MANN-WHITNEY TEST
            if (length(class_list) == 2) {
              wilcoxon_test <-
                wilcox.test(intensity_vector[[1]], intensity_vector[[2]])
            }
            # KRUSKAL-WALLIS TEST
            if (length(class_list) >= 2) {
              kruskal_wallis_test <-
                kruskal.test(signal_matrix[, p], g = as.factor(signal_matrix[, ncol(signal_matrix)]))
            }
            ######################################## Fill the matrix with the values
            # Distribution Type
            ...
            for (l in 1:length(class_list)) {
              if (is.null(distribution_type_name)) {
                distribution_type_name <-
                  paste0(distribution_type[[l]], " - ", class_list[l])
              } else {
                distribution_type_name <-
                  paste0(distribution_type_name,
                         " , ",
                         distribution_type[[l]],
                         " - ",
                         class_list[l])
            ...
            for (l in 1:length(class_list)) {
              if (is.null(mean_intensity_name)) {
                mean_intensity_name <-
                  paste0(mean_intensity[[l]], " - ", class_list[l])
              } else {
                mean_intensity_name <-
                  paste0(mean_intensity_name,
                         " , ",
                         mean_intensity[[l]],
                         " - ",
                         class_list[l])
            ...
            for (l in 1:length(class_list)) {
              if (is.null(coeff_variation_name)) {
                coeff_variation_name <-
                  paste0(coeff_variation[[l]], " - ", class_list[l])
              } else {
                coeff_variation_name <-
                  paste0(coeff_variation_name,
                         " , ",
                         coeff_variation[[l]],
                         " - ",
                         class_list[l])
            ...
            for (l in 1:length(class_list)) {
              if (is.null(median_intensityensity_name)) {
                median_intensityensity_name <-
                  paste0(median_intensityensity[[l]], " - ", class_list[l])
              } else {
                median_intensityensity_name <-
                  paste0(
                    median_intensityensity_name,
            ...
            for (l in 1:length(class_list)) {
              if (is.null(inter_quartile_range_name)) {
                inter_quartile_range_name <-
                  paste0(inter_quartile_range[[l]], " - ", class_list[l])
              } else {
                inter_quartile_range_name <-
                  paste0(
                    inter_quartile_range_name,
            ...
            for (l in 1:length(class_list)) {
              if (is.null(spectra_counter_name)) {
                spectra_counter_name <-
                  paste0(spectra_counter[[l]], " - ", class_list[l])
              } else {
                spectra_counter_name <-
                  paste0(spectra_counter_name,
                         " , ",
                         spectra_counter[[l]],
                         " - ",
                         class_list[l])
            ...
            if (variance_test_parametric$p.value < 0.05) {
              homoscedasticity_parametric <-
                paste(
                  "Non homoscedastic data",
                  "(p-value:",
                  variance_test_parametric$p.value,
                  ")"
                )
            ...
            if (variance_test_non_parametric$p.value < 0.05) {
              homoscedasticity_non_parametric <-
                paste(
                  "Non homoscedastic data",
                  "(p-value:",
                  variance_test_non_parametric$p.value,
                  ")"
                )
            ...
            peak_stat_matrix [p, 10] <-
              homoscedasticity_non_parametric
            # t-Test
            peak_stat_matrix [p, 11] <- t_test$p.value
            # ANOVA
            peak_stat_matrix [p, 12] <-
    ...
    function(spectra,
      ...
      binning_subfunction <-
        function(spectra,
          ...
          data_points_per_bin <-
            floor(length(spectra@mass) / final_data_points)
          # Define the indexes
          index1 <- 1
          ...
          for (d in 1:final_data_points) {
            ...
            for (i in index1:index2) {
              ...
              bin_intensity <-
                append(bin_intensity, spectra@intensity[i])
            }
            ...
            } else if (binning_method == "RMS" |
                       binning_method == "rms") {
              data_point_intensity <- sqrt(sum(bin_intensity ^ 2))
            }
            # Store it in the new spectra_binned list
            spectra_binned@mass <-
              append(spectra_binned@mass, data_point_mass)
            spectra_binned@intensity <-
              append(spectra_binned@intensity, data_point_intensity)
      ...
      if (isMassSpectrumList(spectra)) {
        ...
        for (s in 1:length(spectra)) {
          if (is.null(lowest_data_points) ||
              length(spectra[[s]]@mass) < lowest_data_points) {
        ...
        if ((equality_data_points == 1) &&
            (datapoints_dataset == final_data_points)) {
          spectra_binned <- spectra
        }
          ...
          if ((is.logical(allow_parallelization) &&
            ...
            if (Sys.info()[1] == "Linux" ||
              ...
              spectra_binned <-
                mclapply(
                  spectra,
                  FUN = function (spectra)
                    binning_subfunction(spectra, final_data_points, binning_method),
                  mc.cores = cpu_thread_number
                )
            } else if (Sys.info()[1] == "Windows") {
              #cpu_thread_number <- cpu_thread_number - 1
              ...
              clusterExport(
                cl = cl,
                varlist = c("final_data_points", "binning_method"),
                envir = environment()
              )
              spectra_binned <-
                parLapply(
            ...
            spectra_binned <- list()
            spectra_binned <-
              foreach(i = 1:length(spectra),
                      .packages = "MALDIquant") %dopar% {
                        spectra_binned[[i]] <-
                          binning_subfunction(spectra[[i]],
                                              final_data_points = final_data_points,
        ...
        cat(paste("\nEqual distance between datapoints\n", (all(
          sapply(spectra_binned, isRegular)
        ))))
      } else {
  ...
  replace_backslash <<-
    function(spectra, allow_parallelization = FALSE) {
      ...
      if (isMassSpectrumList(spectra) || isMassPeaksList(spectra)) {
        ...
        if ((is.logical(allow_parallelization) &&
          ...
          if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
            ...
            spectra <-
              mclapply(
                spectra,
                FUN = function(spectra)
                  backslash_replacing_subfunction(spectra),
                mc.cores = cpu_thread_number
              )
          } else if (Sys.info()[1] == "Windows") {
            #cpu_thread_number <- cpu_thread_number - 1
            ...
            clusterExport(
              cl = cl,
              varlist = c("spectra", "spectra_format"),
              envir = environment()
            )
            # Apply the multicore function
            spectra <-
          ...
          spectra <-
            foreach(i = 1:length(spectra), .packages = "MALDIquant") %dopar% {
  ...
  import_spectra <<-
    function(filepath,
             ...
             allow_parallelization = FALSE,
             spectral_names = "name",
             replace_sample_name_field = TRUE,
             remove_empty_spectra = TRUE) {
      ### Load the packages
      require(XML)
      require(MALDIquant)
      ...
      try({
        ...
        if (spectra_format == "imzML") {
          ...
          if (!is.null(mass_range)) {
            spectra <-
              importImzMl(filepath,
                          massRange = mass_range,
                          removeEmptySpectra = FALSE)
          } else {
            # No mass range specified
            spectra <-
              importImzMl(filepath, removeEmptySpectra = FALSE)
          }
        } else if (spectra_format == "fid") {
          ### Xmass
          # Mass range specified
          if (!is.null(mass_range)) {
            spectra <-
              importBrukerFlex(filepath,
                               massRange = mass_range,
                               removeEmptySpectra = FALSE)
          } else {
            # No mass range specified
            spectra <-
              importBrukerFlex(filepath, removeEmptySpectra = FALSE)
          }
        ...
        spectra <-
          replace_backslash(spectra, allow_parallelization = allow_parallelization)
        ### Identify the spectra by setting the names to the spectral list
        spectra <-
          replace_sample_name_list(
            spectra,
            spectra_format = spectra_format,
            type = spectral_names,
            replace_sample_name_field = replace_sample_name_field
          )
  ...
  replace_sample_name <<-
    function(spectra,
      ...
      spectra <-
        replace_backslash(spectra, allow_parallelization = allow_parallelization)
      ##### Function for lapply
      name_replacing_subfunction <-
        function(spectra, spectra_format) {
          if (spectra_format == "imzML" ||
              spectra_format == "txt" ||
              spectra_format == "csv" || spectra_format == "msd") {
            # Split the filepath at /
            sample_name <-
              unlist(strsplit(spectra@metaData$file[1], "/"))
            # The sample name is the last part of the path
            sample_name <- sample_name[length(sample_name)]
            # Detach the file extension
            sample_name <-
              unlist(strsplit(sample_name, paste0(".", spectra_format)))
      ...
      if (isMassSpectrumList(spectra) || isMassPeaksList(spectra)) {
        ...
        if ((is.logical(allow_parallelization) &&
          ...
          if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
            ...
            spectra <-
              mclapply(
                spectra,
                FUN = function(spectra)
                  name_replacing_subfunction(spectra, spectra_format = spectra_format),
                mc.cores = cpu_thread_number
              )
          } else if (Sys.info()[1] == "Windows") {
            #cpu_thread_number <- cpu_thread_number - 1
          ...
          spectra <-
            foreach(i = 1:length(spectra), .packages = "MALDIquant") %dopar% {
              spectra[[i]] <-
                name_replacing_subfunction(spectra[[i]], spectra_format = spectra_format)
            }
          names(spectra) <- list_names
        } else {
          spectra <-
  ...
  group_spectra_class <<-
    function(spectra,
             ...
             spectra_format = "imzML",
             class_in_file_path = TRUE,
             class_in_file_name = FALSE,
             tof_mode = "linear",
             preprocessing_parameters = list(
               mass_range = NULL,
               transformation_algorithm = NULL,
      ...
      if (!is.null(preprocessing_parameters) &&
          is.list(preprocessing_parameters) &&
          length(preprocessing_parameters) > 0) {
        spectra <-
          preprocess_spectra(
            spectra,
            tof_mode = tof_mode,
      ...
      spectra <-
        replace_class_name(
          ...
          class_list = class_list,
          spectra_format = spectra_format,
          class_in_file_path = class_in_file_path,
          class_in_file_name = class_in_file_name
        )
      ...
      if (grouping_method == "mean" || grouping_method == "average") {
        class_spectra_grouped <-
          averageMassSpectra(spectra, labels = class_vector, method = "mean")
      } else if (grouping_method == "skyline") {
        # Skyline
        class_spectra_grouped <-
          averageMassSpectra(spectra, labels = class_vector, method = "sum")
      }
      ...
      if (is.null(names(spectra))) {
        ...
        if (type == "name") {
          ...
          if (replace_sample_name_field == TRUE) {
            spectra <-
              replace_sample_name(spectra,
                                  spectra_format = spectra_format,
                                  allow_parallelization = allow_parallelization)
            ##### More spectra
            if (isMassSpectrumList(spectra) ||
                isMassPeaksList(spectra)) {
              # Generate a vector with all the spectra names
              spectra_names_vector <- character()
              for (s in 1:length(spectra)) {
                spectra_names_vector <-
                  append(spectra_names_vector, spectra[[s]]@metaData$file)
              }
              # Extract the unique values
              if (length(spectra_names_vector) != length(unique(spectra_names_vector))) {
                spectra_names_vector <-
                  make.names(spectra_names_vector, unique = TRUE)
              }
            ...
            if (isMassSpectrumList(spectra) ||
              ...
              for (s in 1:length(spectra)) {
                spectra_names_vector <-
                  append(spectra_names_vector,
                         spectra_temp[[s]]@metaData$file)
              }
              # Extract the unique values
              if (length(spectra_names_vector) != length(unique(spectra_names_vector))) {
                spectra_names_vector <-
  ...
  replace_class_name <<-
    function(spectra,
      ...
      if (!is.null(class_list) || length(class_list) > 0) {
        ...
        if (class_in_file_path == TRUE &&
        ...
        } else if (class_in_file_path == FALSE &&
                   class_in_file_name == TRUE) {
          ...
          if ((isMassSpectrumList(spectra) ||
               isMassPeaksList(spectra)) &&
              (length(spectra) == length(class_list))) {
            for (w in 1:length(class_list)) {
              spectra[[w]]@metaData$file <- class_list[w]
            }
          } else if ((isMassSpectrum(spectra) ||
                      isMassPeaks(spectra)) && (length(class_list) == 1)) {
          ...
          if (spectra_format == "imzML" ||
            ...
            if (isMassSpectrumList(spectra) ||
              ...
              for (i in 1:length(spectra)) {
                ...
                class_name <-
                  unlist(strsplit(spectra[[i]]@metaData$file[1], "/"))
                # The sample name is the last part of the path
  ...
  plot_spectra <<- function(spectra, mass_range = NULL) {
    ...
    if (isMassSpectrum(spectra)) {
      ### Plot the spectrum
      spectral_plot <-
        spectral_plotting_subfunction(spectra, mass_range)
  ...
  preprocess_spectra <<-
    function(spectra,
             ...
             preprocessing_parameters = list(
               ...
               spectral_alignment_reference = "average_spectrum"
             ),
             allow_parallelization = FALSE,
             tolerance_ppm = NULL) {
      ##### Load the required libraries
      ...
      transformation_algorithm <-
        preprocessing_parameters$transformation_algorithm
      smoothing_algorithm <-
        preprocessing_parameters$smoothing_algorithm
      smoothing_strength <-
        preprocessing_parameters$smoothing_strength
      baseline_subtraction_algorithm <-
        preprocessing_parameters$baseline_subtraction_algorithm
      baseline_subtraction_algorithm_parameter <-
        preprocessing_parameters$baseline_subtraction_algorithm_parameter
      normalization_algorithm <-
        preprocessing_parameters$normalization_algorithm
      normalization_mass_range <-
        preprocessing_parameters$normalization_mass_range
      preprocess_spectra_in_packages_of <-
        preprocessing_parameters$preprocess_spectra_in_packages_of
      spectral_alignment_algorithm <-
        preprocessing_parameters$spectral_alignment_algorithm
      spectral_alignment_reference <-
        preprocessing_parameters$spectral_alignment_reference
      ##### Fix the names
      if (!is.null(smoothing_algorithm) &&
          (
            smoothing_algorithm == "SavitzkyGolay" ||
            smoothing_algorithm == "Savitzky-Golay" ||
            smoothing_algorithm == "SG"
          )) {
        smoothing_algorithm <- "SavitzkyGolay"
      } else if (!is.null(smoothing_algorithm) &&
                 (
                   smoothing_algorithm == "MovingAverage" ||
                   smoothing_algorithm == "Moving Average" ||
                   smoothing_algorithm == "MA"
                 )) {
        smoothing_algorithm <- "MovingAverage"
      }
      ...
      if (tof_mode == "linear") {
        ...
        } else if (!is.null(smoothing_strength) &&
                   smoothing_strength == "medium") {
          if (!is.null(smoothing_algorithm) &&
              smoothing_algorithm == "SavitzkyGolay") {
      ...
      preprocessing_subfunction <-
        function(x,
                 mass_range,
                 transformation_algorithm,
                 smoothing_algorithm,
                 smoothing_half_window_size,
                 baseline_subtraction_algorithm,
                 baseline_subtraction_algorithm_parameter,
                 normalization_algorithm,
                 normalization_mass_range) {
          ...
          if (!is.null(smoothing_algorithm)) {
            x <-
              smoothIntensity(x, method = smoothing_algorithm, halfWindowSize = smoothing_half_window_size)
          }
          ### Baseline removal
          if (!is.null(baseline_subtraction_algorithm) &&
              baseline_subtraction_algorithm == "TopHat") {
            # Default value for the half window size
            if (is.null(baseline_subtraction_algorithm_parameter) ||
                baseline_subtraction_algorithm_parameter <= 0) {
              baseline_subtraction_algorithm_parameter <- 100
            }
            x <-
              removeBaseline(x, method = baseline_subtraction_algorithm, halfWindowSize = baseline_subtraction_algorithm_parameter)
          } else if (!is.null(baseline_subtraction_algorithm) &&
                     baseline_subtraction_algorithm == "SNIP") {
            ...
            x <-
              removeBaseline(x, method = baseline_subtraction_algorithm, iterations = baseline_subtraction_algorithm_parameter)
          } else if (!is.null(baseline_subtraction_algorithm) &&
                     baseline_subtraction_algorithm == "median") {
            # Default value for the number of iterations
            if (is.null(baseline_subtraction_algorithm_parameter) ||
                baseline_subtraction_algorithm_parameter <= 0) {
              baseline_subtraction_algorithm_parameter <- 100
      ...
      if (isMassSpectrumList(spectra)) {
        ...
        if (is.null(preprocess_spectra_in_packages_of) ||
            preprocess_spectra_in_packages_of <= 0 ||
        ...
        number_of_folds <-
          as.integer(ceiling(length(spectra) / preprocess_spectra_in_packages_of))
        # Define to which fold the spectra belong to
        if (length(number_of_folds) > 1) {
          spectra_id_folds <-
            cut(seq(1, length(spectra)), breaks = number_of_folds, labels = FALSE)
        ...
        for (f in 1:number_of_folds) {
          ...
          if ((is.logical(allow_parallelization) &&
            ...
            if (Sys.info()[1] == "Linux" ||
              ...
              spectra_temp <-
                mclapply(
                  spectra_temp,
                  FUN = function(spectra_temp)
                    preprocessing_subfunction(
                      spectra_temp,
                      mass_range = mass_range,
                      transformation_algorithm = transformation_algorithm,
          ...
          preprocessed_spectra <-
            append(preprocessed_spectra, spectra_temp)
        }
      } else if (isMassSpectrum(spectra)) {
        #################### Single spectrum
        spectra <-
          ...
          if (!is.null(spectral_alignment_reference) &&
            ...
            aligned_spectra <-
              alignSpectra(
                spectra,
                noiseMethod = "MAD",
                halfWindowSize = half_window_alignment,
                SNR = 2,
                tolerance = (tolerance_ppm / 10 ^ 6),
                warpingMethod = spectral_alignment_algorithm
              )
          } else if (!is.null(spectral_alignment_reference) &&
                     spectral_alignment_reference == "average spectrum") {
            ##### Perform the alignment: Average spectrum
            # Ganerate the average spectrum
            average_spectrum <-
              averageMassSpectra(spectra, method = "mean")
            ...
            average_spectrum <-
              preprocess_spectra(
                ...
                preprocessing_parameters = list(
                  mass_range = NULL,
                  transformation_algorithm = NULL,
                  smoothing_algorithm = smoothing_algorithm_avg,
                  smoothing_strength = "medium",
                  baseline_subtraction_algorithm = "SNIP",
            ...
            if (deisotope_peaklist == TRUE &&
                tof_mode == "reflectron" && envelope_peaklist == FALSE) {
              average_spectrum_peaks <- deisotope_peaks(average_spectrum_peaks)
            } else if (deisotope_peaklist == FALSE &&
                       tof_mode == "reflectron" && envelope_peaklist == TRUE) {
            ...
            aligned_spectra <-
              alignSpectra(
                ...
                tolerance = (tolerance_ppm / 10 ^ 6),
                warpingMethod = spectral_alignment_algorithm,
                reference = average_spectrum_peaks
              )
          } else if (!is.null(spectral_alignment_reference) &&
                     spectral_alignment_reference == "skyline spectrum") {
            ##### Perform the alignment: Skyline spectrum
            # Ganerate the skyline spectrum
            skyline_spectrum <-
              averageMassSpectra(spectra, method = "sum")
            ...
            skyline_spectrum <-
              preprocess_spectra(
                ...
                preprocessing_parameters = list(
                  mass_range = NULL,
                  transformation_algorithm = NULL,
                  smoothing_algorithm = smoothing_algorithm_skyline,
                  smoothing_strength = "medium",
                  baseline_subtraction_algorithm = "SNIP",
                  baseline_subtraction_algorithm_parameter = 200,
                  normalization_algorithm = "TIC",
            ...
            aligned_spectra <-
              alignSpectra(
                ...
                tolerance = (tolerance_ppm / 10 ^ 6),
                warpingMethod = spectral_alignment_algorithm,
                reference = skyline_spectrum_peaks
              )
          }
          ### Check if NA intensities/mz/snr are generated: if yes, go back to the original spectra
          NA_values <- FALSE
          for (s in 1:length(aligned_spectra)) {
            if (length(which(is.na(aligned_spectra[[s]]@mass))) > 0 ||
                length(which(is.na(aligned_spectra[[s]]@intensity))) > 0) {
              NA_values <- TRUE
              break
            }
          }
  ...
  average_spectrum_bars <<-
    function(spectra,
             SNR = 5,
             peak_picking_algorithm = "SuperSmoother",
             tof_mode = "linear",
             tolerance_ppm = NULL,
             mass_range_plot = c(4000, 15000),
             graph_title = "Spectrum",
             average_spectrum_color = "black",
             peak_points = "yes",
             points_color = "red",
             bar_width = 40,
             bar_color = "blue") {
      ...
      average_spectrum <-
        removeBaseline(average_spectrum, method = "TopHat")
      ...
      peaks_all <-
        align_and_filter_peaks(
          peaks_all,
          tof_mode = tof_mode,
          peak_filtering_frequency_threshold_percent = 5,
          low_intensity_peak_removal_threshold_percent = 0,
          tolerance_ppm = tolerance_ppm
        )
      # Empty the lists
        ...
        for (p in 1:length(peaks)) {
          for (i in 1:length(peaks[[p]]@mass)) {
            if (abs(peaks[[p]]@mass[i] == peaks_average@mass[a])) {
              intensity_vector <-
                append(intensity_vector, peaks[[p]]@intensity[i])
            }
          }
        }
        st_dev_intensity <-
          append(st_dev_intensity, sd(intensity_vector))
      }
      ####### Plot
      # Spectrum
      plot(
        average_spectrum,
        main = graph_title,
        col.main = average_spectrum_color,
        xlab = "m/z",
        ylab = "Intensity (a.i.)",
        xlim = mass_range_plot,
        col = average_spectrum_color
      )
      # Peaks
      if (peak_points == "yes") {
      ...
      for (i in 1:length(peaks_average@mass)) {
        ...
        segments(peaks_average@mass[i] - epsilon,
                 low,
                 peaks_average@mass[i] + epsilon,
                 low,
                 col = bar_color)
        segments(peaks_average@mass[i] - epsilon,
                 up,
    ...
    function(spectra,
      ...
      if (isMassSpectrumList(spectra)) {
        ...
        if (spectra_format == "fid") {
          ...
          for (f in 1:length(spectra_files_splitted)) {
            treatment_name <-
              append(treatment_name, spectra_files_splitted[[f]][2])
          }
          ...
          for (p in 1:length(sample_name)) {
            unique_sample_name <-
              append(unique_sample_name,
                     paste0(sample_name[p], "/", treatment_name[p], "/"))
          }
          ...
          for (s in 1:length(spectra)) {
            for (f in 1:length(unique_sample_name)) {
              if (length(grep(
                unique_sample_name[f],
                spectra[[s]]@metaData$file[1],
                fixed = TRUE
              )) > 0) {
        ...
        if (length(spectra_names) == length(spectra)) {
          if (averaging_method == "mean" || averaging_method == "average") {
            spectra_replicates_averaged <-
              averageMassSpectra(spectra, labels = spectra_names, method = "mean")
          } else if (averaging_method == "skyline" ||
                     averaging_method == "sum") {
            spectra_replicates_averaged <-
              averageMassSpectra(spectra, labels = spectra_names, method = "sum")
          }
  ...
  peak_picking <<-
    function(spectra,
      ...
      if (isMassSpectrum(spectra)) {
        ...
        # Peak detection
        if (((
          is.logical(allow_parallelization) &&
          allow_parallelization == TRUE
        ) ||
        (
          is.character(allow_parallelization) &&
      ...
      ###### Deisotope peaklist
      if (deisotope_peaklist == TRUE &&
          envelope_peaklist == FALSE && tof_mode == "reflectron") {
        peaks <-
          deisotope_peaks(
            peaks,
            pattern_model_correlation = 0.95,
            isotopic_tolerance = 10 ^ (-4),
            isotope_pattern_distance = 1.00235,
            isotopic_pattern_size = 3L:10L,
            allow_parallelization = allow_parallelization
          )
      }
      ###### Envelope peaklist
      ...
      subselect_peaks_subfunction <-
        function(peaks, signals_to_take) {
          ...
          if (signals_to_take > 0 &&
              signals_to_take < length(peaks@mass)) {
            # Sort the dataframe according to the SNR
            peaks_data_frame <-
              peaks_data_frame[order(-peaks_data_frame$SNR), ]
            ...
            selected_signals <-
              selected_signals[order(selected_signals$mass), ]
      ...
      if (isMassPeaksList(peaks)) {
        if ((is.logical(allow_parallelization) &&
          ...
          if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
            ...
            peaks <-
              mclapply(
                peaks,
                FUN = function(peaks)
                  subselect_peaks_subfunction(peaks, signals_to_take = signals_to_take),
                mc.cores = cpu_thread_number
              )
          } else if (Sys.info()[1] == "Windows") {
            #cpu_thread_number <- cpu_thread_number - 1
          ...
          most_intense_peaks <-
            foreach(i = 1:length(peaks), .packages = "MALDIquant") %dopar% {
              most_intense_peaks[[i]] <-
                subselect_peaks_subfunction(peaks[[i]], signals_to_take = signals_to_take)
            }
  ...
  normalize_spectra <<-
    function(spectra,
          ...
          if (!is.null(normalization_algorithm)) {
            if (normalization_algorithm == "RMS") {
              ...
              if (!is.null(normalization_mass_range) &&
                  is.numeric(normalization_mass_range)) {
                root_mean_square <-
                  sqrt(sum((x@intensity[intersect(
                    which(x@mass > normalization_mass_range[1]),
                    which(x@mass < normalization_mass_range[2])
                  )]) ^ 2, na.rm = TRUE))
              } else {
              ...
              if (!is.null(normalization_mass_range) &&
                  is.numeric(normalization_mass_range)) {
                x <-
                  calibrateIntensity(x, method = "TIC", range = normalization_mass_range)
              } else {
                x <- calibrateIntensity(x, method = "TIC")
              }
            } else if (normalization_algorithm == "PQN" ||
                       normalization_algorithm == "median") {
      ...
      if (isMassSpectrumList(spectra)) {
        ...
        spectra <-
          normalization_subfunction(spectra,
                                    normalization_algorithm,
                                    normalization_mass_range)
      }
      ##### Return
        ...
        if ((is.logical(allow_parallelization) &&
          ...
          if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
            ...
            peaks_deisotoped <-
              monoisotopicPeaks(
                peaks,
                minCor = pattern_model_correlation,
                tolerance = isotopic_tolerance,
                distance = isotope_pattern_distance,
                size = isotopic_pattern_size,
                mc.cores = cpu_thread_number
              )
          } else if (Sys.info()[1] == "Windows") {
            #cpu_thread_number <- cpu_thread_number - 1
            ...
            clusterExport(
              ...
              varlist = c(
                "peaks",
                "pattern_model_correlation",
                "isotopic_tolerance",
                "isotope_pattern_distance",
                "isotopic_pattern_size"
              ),
              envir = environment()
            )
            # Apply the multicore function
  ...
  envelope_peaks <<-
    function(peaks, allow_parallelization = FALSE) {
      ##### Load the required packages
      require(XML)
      require(MALDIquant)
      require(parallel)
        ...
        for (int in 1:length(intensity_values)) {
          ...
          if (int == 1) {
            if ((intensity_values[int + 1] < intensity_values[int]) &&
                (mz_values[int + 1] - mz_values[int] <= 1.1)) {
            ...
            if ((
              intensity_values[int - 1] < intensity_values[int] &&
              intensity_values[int + 1] < intensity_values[int]
            ) &&
            (mz_values[int - 1] - mz_values[int] <= 1.1 &&
             mz_values[int + 1] - mz_values[int] <= 1.1)) {
          ...
          if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
            ...
            peaks_enveloped <-
              mclapply(
                peaks,
                FUN = function(peaks)
                  envelope_peaklist_subfunction(peaks),
                mc.cores = cpu_thread_number
              )
          } else if (Sys.info()[1] == "Windows") {
            #cpu_thread_number <- cpu_thread_number - 1
  ...
  align_and_filter_peaks <<-
    function(peaks,
             ...
             tof_mode = "linear",
             peak_filtering_frequency_threshold_percent = 5,
             class_vector_for_peak_filtering = NULL,
             low_intensity_peak_removal_threshold_percent = 0,
             low_intensity_peak_removal_threshold_method = "element-wise",
             reference_peaklist = NULL,
             spectra = NULL,
      ...
      if (isMassPeaksList(peaks)) {
        ...
        if (alignment_iterations <= 0 ||
            !is.numeric(alignment_iterations)) {
          alignment_iterations <- 5
        }
        # Retain the names
        if (!is.null(names(peaks))) {
        ...
        if (!is.null(low_intensity_peak_removal_threshold_percent) &&
            low_intensity_peak_removal_threshold_percent > 0 &&
            low_intensity_peak_removal_threshold_percent < 100) {
          peaks_aligned <-
            remove_low_intensity_peaks(
        ...
        if (!is.null(peak_filtering_frequency_threshold_percent) &&
            peak_filtering_frequency_threshold_percent > 0) {
          if (is.null(class_vector_for_peak_filtering)) {
            peaks_aligned <-
              filterPeaks(
                peaks_aligned,
                minFrequency = (peak_filtering_frequency_threshold_percent / 100)
              )
          } else {
            if (!is.factor(class_vector_for_peak_filtering)) {
              class_vector_for_peak_filtering <-
                as.factor(class_vector_for_peak_filtering)
            }
            peaks_aligned <-
              filterPeaks(
                peaks_aligned,
                minFrequency = (peak_filtering_frequency_threshold_percent / 100),
                labels = class_vector_for_peak_filtering
              )
          }
        }
        names(peaks_aligned) <- peaks_names
        ##### Align to a reference peaklist: AVERAGE SPECTRUM or SKYLINE SPECTRUM (if a spectra list is provided)
        if (is.character(reference_peaklist) &&
            reference_peaklist == "average spectrum" && !is.null(spectra)) {
          # Average the spectra
          average_spectrum <-
            averageMassSpectra(spectra, method = "mean")
          # Peak picking
          ...
          reference_peaklist <-
            createMassPeaks(
              mass = average_spectrum_peaks@mass,
              intensity = average_spectrum_peaks@intensity,
              snr = rep.int(5, length(average_spectrum_peaks@mass)),
              metaData = list(name = "Reference peaklist AVG")
            )
        } else if (is.character(reference_peaklist) &&
                   reference_peaklist == "skyline spectrum" && !is.null(spectra)) {
          # Average the spectra
          skyline_spectrum <-
            averageMassSpectra(spectra, method = "sum")
          # Peak picking
          ...
          reference_peaklist <-
            createMassPeaks(
              mass = skyline_spectrum_peaks@mass,
              intensity = skyline_spectrum_peaks@intensity,
              snr = rep.int(5, length(skyline_spectrum_peaks@mass)),
              metaData = list(name = "Reference peaklist Skyline")
            )
        } else if (is.character(reference_peaklist) &&
                   (
                     reference_peaklist == "average spectrum" ||
                     reference_peaklist == "skyline spectrum"
                   ) && is.null(spectra)) {
          reference_peaklist <- NULL
        } else if (!is.null(reference_peaklist) &&
                   is.vector(reference_peaklist)) {
          reference_peaklist <-
            createMassPeaks(
              mass = as.numeric(reference_peaklist),
              intensity = rep.int(1, length(reference_peaklist)),
              snr = rep.int(5, length(reference_peaklist)),
              metaData = list(name = "Reference peaklist")
            )
        }
              ...
              if ((is.logical(allow_parallelization) &&
                ...
                if (Sys.info()[1] == "Linux" ||
                  ...
                  peaks_aligned <-
                    mclapply(
                      peaks_aligned,
                      FUN = function(peaks_aligned)
                        warpMassPeaks(peaks_aligned, w = warping_functions),
                      mc.cores = cpu_thread_number
                    )
                } else if (Sys.info()[1] == "Windows") {
                  #cpu_thread_number <- cpu_thread_number - 1
                  ...
                  clusterExport(
                    cl = cl,
                    varlist = c("reference_peaklist", "tolerance_ppm"),
                    envir = environment()
                  )
                  peaks_aligned <-
                    parLapply(
                ...
                peaks_aligned <-
                  foreach(i = 1:length(peaks_aligned),
                          .packages = "MALDIquant") %dopar% {
                            peaks_aligned[[i]] <-
                              warpMassPeaks(peaks_aligned[[i]], w = warping_functions)
                          }
  ...
  single_model_classification_of_spectra <<-
    function(spectra,
             ...
             tof_mode = "linear",
             allow_parallelization = FALSE,
             pixel_grouping = c("single", "hca", "moving window average", "graph"),
             number_of_hca_nodes = 5,
             moving_window_size = 5,
             seed = 12345,
             correlation_method_for_adjacency_matrix = "pearson",
             correlation_threshold_for_adjacency_matrix = 0.95,
             pvalue_threshold_for_adjacency_matrix = 0.05,
             max_GA_generations = 10,
             iterations_with_no_change = 5,
             classification_mode_graph = c("average spectra", "single spectra clique"),
             plot_figures = TRUE,
             plot_graphs = TRUE,
             plot_legends = c("sample name", "legend", "plot name"),
             features_to_use_for_graph = c("all", "model"),
             tolerance_ppm = NULL) {
      ...
      if (isMassSpectrumList(spectra)) {
        ...
        if (pixel_grouping == "single" ||
            number_of_hca_nodes == 1 ||
            moving_window_size >= length(spectra)) {
          ### Generate the intensity matrix with the features from the model
          final_sample_matrix <-
            generate_custom_intensity_matrix(
              spectra,
              custom_feature_vector = features_model,
              tof_mode = tof_mode,
              preprocessing_parameters = preprocessing_parameters,
              peak_picking_algorithm = peak_picking_algorithm,
              peak_picking_SNR = peak_picking_SNR,
              peak_filtering_frequency_threshold_percent = peak_filtering_frequency_threshold_percent,
              low_intensity_peak_removal_threshold_percent = low_intensity_peak_removal_threshold_percent,
              low_intensity_peak_removal_threshold_method = low_intensity_peak_removal_threshold_method,
              allow_parallelization = allow_parallelization,
              deisotope_peaklist = deisotope_peaklist,
              envelope_peaklist = envelope_peaklist,
              tolerance_ppm = tolerance_ppm
            )
          ...
          if (!is.null(final_sample_matrix)) {
            ...
            for (n in 1:length(colnames(final_sample_matrix))) {
              colnames(final_sample_matrix)[n] <-
                paste0("X", colnames(final_sample_matrix)[n])
            }
            ##### Predictions (spectra by spectra)
            predicted_classes <-
              as.character(predict(model_object, newdata = final_sample_matrix))#, type = "raw"))
            names(predicted_classes) <- rownames(final_sample_matrix)
            # Generate a matrix with the results
            result_matrix_model <-
              matrix(nrow = length(predicted_classes),
                     ncol = 1)
            sample_name <- spectra[[1]]@metaData$file[1]
            if (is.null(names(spectra))) {
              rownames(result_matrix_model) <-
                cbind(rep(sample_name, length(spectra)))
            } else {
              rownames(result_matrix_model) <- names(spectra)
            }
            result_matrix_model[, 1] <- cbind(predicted_classes)
            colnames(result_matrix_model) <-
              paste0("Predicted Class '", model_name, "'")
            ...
            outcome_and_class <-
              outcome_and_class_to_MS(
                ...
                class_vector = predicted_classes
              )
            ...
            class_as_number <-
              outcome_and_class$class_vector_as_numeric
            spectra_for_plotting <- spectra
            if (is.null(names(spectra_for_plotting)) ||
                is.null(names(class_as_number))) {
              for (s in 1:length(spectra_for_plotting)) {
                spectra_for_plotting[[s]]@intensity <-
                  rep(class_as_number[s],
                      length(spectra_for_plotting[[s]]@intensity))
              }
            } else {
              for (s in 1:length(spectra_for_plotting)) {
                spectra_for_plotting[[s]]@intensity <-
                  rep(class_as_number[names(spectra_for_plotting)[s]],
                      length(spectra_for_plotting[[s]]@intensity))
              }
            }
            ...
            slices <-
              msiSlices(
                spectra_for_plotting,
                center = spectra_for_plotting[[1]]@mass[(length(spectra_for_plotting[[1]]@mass) /
                                                           2)],
                tolerance = 1,
                adjust = TRUE,
                method = "median"
          ...
          spectra <-
            rearrange_spectral_dataset(spectra, rearranging_method = "space")
          ...
          for (s in 1:length(spectra)) {
            ...
            if (!is.null(sample_bin_matrix)) {
              ...
              for (n in 1:length(colnames(sample_bin_matrix))) {
                colnames(sample_bin_matrix)[n] <-
                  paste0("X", colnames(sample_bin_matrix)[n])
              }
              ##### Predictions (AVG spectrum)
              predicted_class_avg <-
                as.character(predict(model_object, newdata = sample_bin_matrix))#, type = "raw"))
              ...
              result_matrix_model_bin[1, 1] <-
                as.character(predicted_class_avg)
              colnames(result_matrix_model_bin) <-
                paste0("Predicted Class '", model_name, "'")
            } else {
              ### It is NULL if there are incompatibilities between the model features and the spectral features
            ...
            if (is.null(result_matrix_model)) {
              ...
              result_matrix_model <-
                rbind(result_matrix_model, result_matrix_model_bin)
            }
          }
          final_result_matrix <- as.data.frame(result_matrix_model)
          ########## Generate a molecular image of the classification
          ...
          ### Initialize the model result matrix
          result_matrix_model <-
            matrix("class", nrow = length(spectra), ncol = 1)
          colnames(result_matrix_model) <-
            paste0("Predicted Class '", model_name, "'")
          sample_name <- spectra[[1]]@metaData$file[1]
          if (is.null(names(spectra))) {
            rownames(result_matrix_model) <-
              rep(sample_name, nrow(result_matrix_model))
          } else {
            rownames(result_matrix_model) <- names(spectra)
          }
          ...
          for (n in 1:number_of_hca_nodes) {
            ...
            if (!is.null(sample_hca_matrix)) {
              ...
              for (x in 1:length(colnames(sample_hca_matrix))) {
                colnames(sample_hca_matrix)[x] <-
                  paste0("X", colnames(sample_hca_matrix)[x])
              }
              ##### Predictions (AVG spectrum)
              predicted_class_avg <-
                as.character(predict(model_object, newdata = sample_hca_matrix))#, type = "raw"))
              names(predicted_class_avg) <-
                rownames(sample_hca_matrix)
              ...
              if (is.null(spectra_hca_names)) {
                ...
                result_matrix_model[spectra_hca_names, 1] <-
                  as.character(predicted_class_avg)
              }
            } else {
              ### It is NULL if there are incompatibilities between the model features and the spectral features
          ...
          graph_segmentation <-
            graph_MSI_segmentation(
              filepath_imzml = spectra,
              custom_feature_vector = custom_feature_vector_graph,
              preprocessing_parameters = NULL,
              allow_parallelization = allow_parallelization,
              peak_picking_algorithm = peak_picking_algorithm,
              deisotope_peaklist = deisotope_peaklist,
              ...
              low_intensity_peak_removal_threshold_percent = low_intensity_peak_removal_threshold_percent,
              low_intensity_peak_removal_threshold_method = low_intensity_peak_removal_threshold_method,
              correlation_method_for_adjacency_matrix = correlation_method_for_adjacency_matrix,
              correlation_threshold_for_adjacency_matrix = correlation_threshold_for_adjacency_matrix,
              pvalue_threshold_for_adjacency_matrix = pvalue_threshold_for_adjacency_matrix,
              number_of_high_degree_vertices_for_subgraph = 0,
              vertices_not_in_induced_subgraph = "independent",
              max_GA_generations = max_GA_generations,
              iterations_with_no_change = iterations_with_no_change,
          ...
          spectra_independent <-
            graph_segmentation$spectra_independent
          ...
          if (classification_mode_graph == "average spectra") {
            ...
            spectra_independent_avg <-
              averageMassSpectra(spectra_independent)
            # Preprocess the average spectrum
            spectra_clique_avg <-
              preprocess_spectra(
                spectra_clique_avg,
            ...
            if (!is.null(sample_clique_matrix)) {
              ...
              for (x in 1:length(colnames(sample_clique_matrix))) {
                colnames(sample_clique_matrix)[x] <-
                  paste0("X", colnames(sample_clique_matrix)[x])
              }
              ##### Predictions (AVG spectrum)
              predicted_class_avg <-
                as.character(predict(model_object, newdata = sample_clique_matrix))#, type = "raw"))
              names(predicted_classes) <-
                rownames(sample_clique_matrix)
              ### Fix the spectra for plotting list (edit the intensity values)
              # Define the class as number depending on the outcome
              outcome_and_class <-
                outcome_and_class_to_MS(
                  class_list = class_list,
                  outcome_list = outcome_list,
                  class_vector = predicted_class_avg
                )
              # Replace the spectra intensities with the class number for plotting purposes
              class_as_number <-
                outcome_and_class$class_vector_as_numeric
              ...
              for (s in 1:length(spectra_for_plotting_clique)) {
                spectra_for_plotting_clique[[s]]@intensity <-
                  rep(class_as_number,
                      length(spectra_for_plotting_clique[[s]]@intensity))
              }
              # Append to the final list of spectra for plotting
              spectra_for_plotting <-
                append(spectra_for_plotting,
                       spectra_for_plotting_clique)
            } else {
              ### It is NULL if there are incompatibilities between the model features and the spectral features
            ...
            if (!is.null(sample_independent_matrix)) {
              ...
              for (x in 1:length(colnames(sample_independent_matrix))) {
                colnames(sample_independent_matrix)[x] <-
                  paste0("X", colnames(sample_independent_matrix)[x])
              }
              ##### Predictions (AVG spectrum)
              predicted_class_avg <-
                as.character(predict(model_object, newdata = sample_independent_matrix))#, type = "raw"))
              names(predicted_classes) <-
                rownames(sample_independent_matrix)
              ### Fix the spectra for plotting list (edit the intensity values)
              # Define the class as number depending on the outcome
              outcome_and_class <-
                outcome_and_class_to_MS(
              ...
              for (s in 1:length(spectra_for_plotting_independent)) {
                spectra_for_plotting_independent[[s]]@intensity <-
                  rep(
                    class_as_number,
                    length(spectra_for_plotting_independent[[s]]@intensity)
                  )
              }
              # Append to the final list of spectra for plotting
              spectra_for_plotting <-
                append(spectra_for_plotting,
                       spectra_for_plotting_independent)
            } else {
              ### It is NULL if there are incompatibilities between the model features and the spectral features
            ...
            if (length(spectra_for_plotting) > 0) {
              ...
              if ("sample name" %in% plot_legends) {
                legend(
                  x = "topright",
                  legend = spectra_for_plotting[[1]]@metaData$file[1],
                  xjust = 0.5,
                  yjust = 0.5
                )
              }
              ...
              outcome_and_class <-
                outcome_and_class_to_MS(class_list = class_list, outcome_list = outcome_list)
                  ...
                  if (spectra_for_plotting[[s]]@intensity[1] == outcome_and_class$class_outcome_matrix[, "Number"][ou]) {
                    predicted_classes <-
                      append(predicted_classes,
                             outcome_and_class$class_outcome_matrix[, "Class"][ou])
                  }
                }
              }
              names(predicted_classes) <- names(spectra_for_plotting)
              result_matrix_model[, 1] <-
                cbind(as.character(predicted_classes))
              rownames(result_matrix_model) <-
                names(spectra_for_plotting)
              final_result_matrix <-
                as.data.frame(result_matrix_model)
            } else {
              ### It is NULL if there are incompatibilities between the model features and the spectral features
              result_matrix_model <- NULL
              classification_msi_model <- NULL
            ...
            if (!is.null(sample_clique_matrix)) {
              ...
              if (model_ID == "rf" ||
                  model_ID == "nbc" ||
                  model_ID == "knn" ||
                  model_ID == "nnet" || model_ID == "lda" || model_ID == "bayes") {
                predicted_classes <-
                  as.character(predict(model_object, newdata = sample_clique_matrix))#, type = "raw"))
                names(predicted_classes) <-
                  rownames(sample_clique_matrix)
                ...
                predicted_classes <-
                  as.character(predict(model_object, newdata = sample_clique_matrix))
                names(predicted_classes) <-
                  rownames(sample_clique_matrix)
              }
              ### Fix the spectra for plotting list (edit the intensity values)
              ...
              # Append to the final list of spectra for plotting
              spectra_for_plotting <-
                append(spectra_for_plotting_independent,
                       spectra_for_plotting_clique)
            } else {
              ### It is NULL if there are incompatibilities between the model features and the spectral features
              spectra_for_plotting_clique <- list()
            ...
            if (length(spectra_for_plotting) > 0) {
              ...
              classification_msi_model <- recordPlot()
              # Generate the result matrix from the rearranged spectra (so that it is reproducible, because the spectra of the clique mught not be always the same) according to the intensity value
              result_matrix_model_independent <-
                cbind(as.character(rep(
                  "independent", length(spectra_independent)
                )))
              names(result_matrix_model_independent) <-
                names(spectra_independent)
              result_matrix_model_clique <-
                cbind(as.character(predicted_classes))
              names(result_matrix_model_clique) <-
                names(spectra_clique)
              result_matrix_model[, 1] <-
                rbind(result_matrix_model_clique,
                      result_matrix_model_independent)
              ...
              if (is.null(final_result_matrix)) {
                ...
                rownames(final_result_matrix) <-
                  final_result_matrix[, 1]
                final_result_matrix <-
                  final_result_matrix[, 2:ncol(final_result_matrix)]
              }
            } else {
              ### It is NULL if there are incompatibilities between the model features and the spectral features
              result_matrix_model <- NULL
        ...
        return(
          list(
            result_matrix_model = result_matrix_model,
            classification_msi_model = classification_msi_model,
            final_result_matrix = final_result_matrix
          )
        )
      } else if (isMassSpectrum(spectra)) {
        ########## SINGLE (AVG) SPECTRUM
        ...
        final_sample_matrix <-
          generate_custom_intensity_matrix(
            ...
            peak_picking_algorithm = peak_picking_algorithm,
            peak_picking_SNR = 3,
            peak_filtering_frequency_threshold_percent = NULL,
            low_intensity_peak_removal_threshold_percent = 0,
            low_intensity_peak_removal_threshold_method = "element-wise",
            allow_parallelization = allow_parallelization,
            deisotope_peaklist = deisotope_peaklist,
        ...
        if (!is.null(final_sample_matrix)) {
          ...
          colnames(result_matrix) <-
            paste("Predicted Class", model_name)
          ...
          for (f in features_model) {
            ...
            for (z in 1:length(sample_peaks_avg_for_bars@mass)) {
              # If there is a match...
              if ((
                abs(sample_peaks_avg_for_bars@mass[z] - as.numeric(f)) * 10 ^ 6 / as.numeric(f)
              ) <= ifelse(tof_mode == "linear", 2000, 200)) {
                # Add the intensity of this peak to the y coordinates of the bars
                coordinates_of_bars$x = append(coordinates_of_bars$x,
                                               sample_peaks_avg_for_bars@mass[z])
                coordinates_of_bars$y = append(coordinates_of_bars$y,
                                               sample_peaks_avg_for_bars@intensity[z])
          ...
          if (plot_figures == TRUE) {
            ...
            if ("sample name" %in% plot_legends) {
              legend(
                x = "topright",
                legend = spectra@metaData$file[[1]],
                xjust = 0.5,
                yjust = 0.5
              )
            }
            ...
            for (s in 1:length(coordinates_of_bars$x)) {
              ...
              segments(
                coordinates_of_bars$x[s],
                0,
                coordinates_of_bars$x[s],
                coordinates_of_bars$y[s],
                col = "red",
                lwd = 2
              )
              # Horizontal segments(x,y , x,y)
              segments(
                coordinates_of_bars$x[s] - 20,
                0,
                coordinates_of_bars$x[s] + 20,
                0,
                col = "red",
        ...
        return(
          list(
            result_matrix_model = result_matrix,
            average_spectrum_with_bars = average_spectrum_with_bars,
            final_result_matrix = final_result_matrix
          )
        )
      }
  ...
  spectral_classification <<-
    function(spectra_path,
             filepath_R = NULL,
             model_list,
             model_performance_parameter_list = NULL,
             classification_mode = c("pixel", "profile"),
             peak_picking_algorithm = "SuperSmoother",
             deisotope_peaklist = FALSE,
             preprocessing_parameters = list(
               mass_range = c(4000, 15000),
               transformation_algorithm = NULL,
               smoothing_algorithm = "SavitzkyGolay",
               smoothing_strength = "medium",
               baseline_subtraction_algorithm = "SNIP",
             ...
             pixel_grouping = c("single", "moving window average", "graph", "hca"),
             moving_window_size = 5,
             number_of_hca_nodes = 10,
             correlation_method_for_adjacency_matrix = "pearson",
             correlation_threshold_for_adjacency_matrix = 0.95,
             pvalue_threshold_for_adjacency_matrix = 0.05,
             max_GA_generations = 10,
             iterations_with_no_change_GA = 5,
             seed = 12345,
             classification_mode_graph = c("average spectra", "single spectra clique"),
             features_to_use_for_graph = c("all", "model"),
             plot_figures = TRUE,
      ...
      if (length(classification_mode) == 1 &&
          classification_mode == "") {
      ...
      if (!is.list(spectra_path) &&
          length(grep(".imzML", spectra_path, fixed = TRUE)) == 0) {
        filepath_test_imzml <-
          read_spectra_files(spectra_path,
                             spectra_format = "imzML",
                             full_path = TRUE)
      } else if (!is.list(spectra_path) &&
                 length(grep(".imzML", spectra_path, fixed = TRUE)) > 0) {
        ## Single imzML filepath
        filepath_test_imzml <- spectra_path
      } else if (isMassSpectrumList(spectra_path) ||
                 isMassSpectrum(spectra_path)) {
        ## Spectra list
      ...
      for (p in 1:length(filepath_test_imzml)) {
        ...
        if (!isMassSpectrumList(spectra_path) ||
          ...
          sample_spectra <-
            import_spectra(
              filepath = filepath_test_imzml[p],
              spectra_format = "imzML",
              mass_range = mass_range,
              allow_parallelization = allow_parallelization,
              spectral_names = "name",
        ...
        if ("profile" %in% classification_mode) {
          ...
          if (isMassSpectrumList(sample_spectra)) {
            sample_spectra_avg <-
              averageMassSpectra(sample_spectra, method = "mean")
        ...
        if (is.null(model_list) &&
            is.null(model_performance_parameter_list) &&
            !is.null(filepath_R) && is.character(filepath_R)) {
          # Create a temporary environment
          ...
          model_performance_parameter_list <-
            get("model_performance_parameter_list", pos = temporary_environment)
        }
          ...
          if ("pixel" %in% classification_mode) {
            ...
            model_classification <-
              single_model_classification_of_spectra(
                ...
                pvalue_threshold_for_adjacency_matrix = pvalue_threshold_for_adjacency_matrix,
                max_GA_generations = max_GA_generations,
                iterations_with_no_change = iterations_with_no_change_GA,
                plot_figures = plot_figures,
                plot_graphs = plot_graphs,
                plot_legends = plot_legends,
                classification_mode_graph = classification_mode_graph,
                features_to_use_for_graph = features_to_use_for_graph,
                tolerance_ppm = tolerance_ppm
              )
            # MSI classification
            if (plot_figures == TRUE) {
              classification_ms_images_model <-
                model_classification$classification_msi_model
            } else {
            ...
            if (is.null(final_result_matrix_msi_patient)) {
              final_result_matrix_msi_patient <-
                as.data.frame(model_classification$final_result_matrix)
            } else {
              final_result_matrix_msi_patient <-
                merge(
                  final_result_matrix_msi_patient,
              ...
              rownames(final_result_matrix_msi_patient) <-
                final_result_matrix_msi_patient[, 1]
              final_result_matrix_msi_patient <-
                final_result_matrix_msi_patient[, 2:ncol(final_result_matrix_msi_patient)]
            }
            ...
            classification_ms_images_patient[[(list_of_models[md])]] <-
              classification_ms_images_model
          }
          if ("profile" %in% classification_mode) {
            # Perform the classification
            model_classification_profile <-
              single_model_classification_of_spectra(
                spectra = sample_spectra_avg,
                model_x = model_list[[md]],
                model_name = list_of_models[md],
                preprocessing_parameters = NULL,
                peak_picking_algorithm = peak_picking_algorithm,
            ...
            # Plot AVG spectrum
            if (plot_figures == TRUE) {
              average_spectrum_profile_with_bars_model <-
                model_classification_profile$average_spectrum_with_bars
            } else {
              average_spectrum_profile_with_bars_model <- NULL
            }
            # Add the model result matrix to the final matrix (the classification function automatically attach the result to a result matrix if provided)
            if (is.null(final_result_matrix_profile_patient)) {
              final_result_matrix_profile_patient <-
                as.data.frame(model_classification_profile$final_result_matrix)
            } else {
              final_result_matrix_profile_patient <-
                merge(
                  final_result_matrix_profile_patient,
                  as.data.frame(
                    model_classification_profile$final_result_matrix
                  ),
                  by = "row.names",
                  all = TRUE
                )
              rownames(final_result_matrix_profile_patient) <-
                final_result_matrix_profile_patient[, 1]
              final_result_matrix_profile_patient <-
                final_result_matrix_profile_patient[, 2:ncol(final_result_matrix_profile_patient)]
            }
            # Append the classification image list to the final list
            average_spectrum_with_bars_patient[[(list_of_models[md])]] <-
              average_spectrum_profile_with_bars_model
          }
        }
        ...
        if ("pixel" %in% classification_mode) {
          ...
          final_result_matrix_msi_list[[sample_name]] <-
            final_result_matrix_msi_patient
          ##### Store all the MS images in the element of the final list of MS images
          classification_ms_images_list[[sample_name]] <-
            classification_ms_images_patient
        }
        if ("profile" %in% classification_mode) {
          ##### Store the matrix with the classification
          final_result_matrix_profile_list[[sample_name]] <-
            final_result_matrix_profile_patient
          ##### Store the plots with the AVG spectrum with bars
          average_spectrum_with_bars_profile_list[[sample_name]] <-
            average_spectrum_with_bars_patient
        }
        ######################################## ENSEMBLE VOTE
        ...
        ########## Ensemble results
        if ("pixel" %in% classification_mode) {
          if (length(list_of_models) > 2 &&
              !is.null(final_result_matrix_msi_patient) &&
              classes_are_the_same_for_each_model == TRUE &&
              outcomes_are_the_same_for_each_model == TRUE) {
            ##### Unweighted majority
            if ("unweighted majority" %in% decision_method_ensemble) {
              ...
              classification_ensemble_matrix_msi <-
                ensemble_vote_classification(
                  classification_matrix = final_result_matrix_msi_patient,
                  class_list = model_list[[1]]$class_list,
                  weighted_decision_method = "unweighted majority",
                  performance_parameter_list = model_performance_parameter_list,
                  type_of_validation_for_performance_estimation = "cv",
                  alpha = alpha
                )
              # Store the ensemble classification matrix in the final output list
              classification_ensemble_matrix_msi_all[[sample_name]][["unweighted majority"]] <-
                classification_ensemble_matrix_msi
              ### Molecular image of the classification
              # Generate the "predicted classes" vector from the ensemble classification matrix
              predicted_classes <-
                as.character(classification_ensemble_matrix_msi)
              names(predicted_classes) <-
                rownames(classification_ensemble_matrix_msi)
              # Define the class as number depending on the outcome
              outcome_and_class <-
                outcome_and_class_to_MS(
                  class_list = model_list[[1]]$class_list,
                  outcome_list = model_list[[1]]$outcome_list,
                  class_vector = predicted_classes
                )
              # Replace the spectra intensities with the class number for plotting purposes
              class_as_number <-
              ...
              if ("plot name" %in% plot_legends) {
                legend(
                  x = "topleft",
                  legend = "Ensemble classifier",
                  xjust = 0.5,
                  yjust = 0.5
                )
              }
              # Store the plot into the list of images
              classification_ensemble_ms_image_list[[sample_name]][["unweighted majority"]] <-
                recordPlot()
            } else {
              classification_ensemble_matrix_msi_all[[sample_name]][["unweighted majority"]] <-
                NULL
            ...
            if ("bayesian probabilities" %in% decision_method_ensemble) {
              ...
              # Store the ensemble classification matrix in the final output list
              classification_ensemble_matrix_msi_all[[sample_name]][["bayesian probabilities"]] <-
                classification_ensemble_matrix_msi
              ### Molecular image of the classification
              # Generate the "predicted classes" vector from the ensemble classification matrix
              predicted_classes <-
              ...
              # Store the plot into the list of images
              classification_ensemble_ms_image_list[[sample_name]][["bayesian probabilities"]] <-
                recordPlot()
            } else {
              classification_ensemble_matrix_msi_all[[sample_name]][["bayesian probabilities"]] <-
                NULL
            ...
            classification_ensemble_ms_image_list[[sample_name]] <-
              NULL
          }
        }
        if ("profile" %in% classification_mode) {
          if (length(list_of_models) > 2 &&
              !is.null(final_result_matrix_profile_patient) &&
              classes_are_the_same_for_each_model == TRUE &&
              outcomes_are_the_same_for_each_model == TRUE) {
            ########## Ensemble results
            classification_ensemble_matrix_profile <-
              ensemble_vote_classification(
                classification_matrix = final_result_matrix_profile_patient,
                class_list = model_list[[1]]$class_list,
                weighted_decision_method = decision_method_ensemble,
                classification_probabilities_list = predicted_classes_probs_list,
                performance_parameter_list = model_performance_parameter_list,
                type_of_validation_for_performance_estimation = "cv",
                alpha = alpha
              )
            ...
            if (is.null(classification_ensemble_matrix_profile_all)) {
              classification_ensemble_matrix_profile_all <-
                classification_ensemble_matrix_profile
            } else {
              classification_ensemble_matrix_profile_all <-
                rbind(
      ...
      return(
        list(
          final_result_matrix_msi_list = final_result_matrix_msi_list,
          final_result_matrix_profile_list = final_result_matrix_profile_list,
          classification_ms_images_list = classification_ms_images_list,
          classification_ensemble_matrix_msi_all = classification_ensemble_matrix_msi_all,
          classification_ensemble_matrix_profile_all = classification_ensemble_matrix_profile_all,
          classification_ensemble_ms_image_list = classification_ensemble_ms_image_list,
          average_spectrum_with_bars_profile_list = average_spectrum_with_bars_profile_list
        )
      )
    }
  ...
  training_test_split <<-
    function(peaklist,
             discriminant_attribute = "Class",
             seed = 12345,
             percentage_of_observations_for_training = 50,
      ...
      if (length(class_list) == 0 ||
          length(class_list) == 1 || unbalanced_split == TRUE) {
        ...
        index_training <-
          sample.int(
            n = nrow(peaklist),
            size = round(
              nrow(peaklist) * percentage_of_observations_for_training / 100
            )
          )
      } else if (length(class_list) > 1 &&
                 unbalanced_split == FALSE) {
        ...
        index_training <-
          createDataPartition(
            y = peaklist[, discriminant_attribute],
            p = (percentage_of_observations_for_training / 100),
            times = 1,
            list = TRUE
          )[[1]]
      }
  ...
  feature_selection <<-
    function(training_set,
             ...
             selection_method = "pls",
             selection_metric = "Kappa",
             correlation_method = "pearson",
             correlation_threshold = 0.75,
             auc_threshold = 0.7,
             cv_repeats_control = 5,
             k_fold_cv_control = 10,
             discriminant_attribute = "Class",
             non_features = c("Sample", "Class", "THY"),
             seed = NULL,
      ...
      if (feature_selection_method == "ANOVA") {
        feature_list <-
          names(training_set[, !(names(training_set) %in% non_features)])
        ...
        for (feature in feature_list) {
          ...
          feature_ANOVA <-
            aov(intensity_vector ~ training_set[, discriminant_attribute])
          # Extract the p-value
          feature_ANOVA_pvalue <-
            summary(feature_ANOVA)[[1]]$"Pr(>F)"[1]
      ...
      ################################################################ KRUSKAL-WALLIS
      if (feature_selection_method == "kruskal" ||
          feature_selection_method == "kruskal-wallis" ||
          feature_selection_method == "Kruskal-Wallis") {
        feature_list <-
          names(training_set[, !(names(training_set) %in% non_features)])
        ...
        for (feature in feature_list) {
          ...
          # Compute an ANOVA based upon the discriminant attribute
          feature_kruskal <-
            kruskal.test(intensity_vector ~ training_set[, discriminant_attribute])
          # Extract the p-value
      ...
      if (feature_selection_method == "correlation") {
        ...
        training_set_features <-
          training_set[, !(names(training_set) %in% non_features)]
        # Compute the correlation
        feature_correlation <-
          cor(training_set_features, method = correlation_method)
        # Output the highly correlated features
        highly_correlated <-
          findCorrelation(feature_correlation, correlation_threshold)
        # List the highly correlated features
        highly_correlated_features <-
          names(training_set_features[, highly_correlated])
        # Features to keep
        low_correlation_features <-
          names(training_set_features[, -highly_correlated])
        cat(paste(
          "\nThe number of selected features is",
          length(low_correlation_features),
          "out of",
          length(training_set_features)
        ))
        # Predictors
      ...
      if (feature_selection_method == "importance") {
        ...
        feature_model <-
          train(
            x = training_set[, !(names(training_set) %in% non_features)],
            y = as.factor(training_set[, discriminant_attribute]),
            method = "pls",
            preProcess = "scale",
        ...
        feature_importance_df$Features <-
          rownames(feature_importance_df)
        feature_importance_df <-
          feature_importance_df [order(-feature_importance_df$Overall), ]
        # Predictors
        predictors_feature_selection <-
          feature_importance_df$Features [1:features_to_select]
      }
      ########################################################################### ROC
      if (feature_selection_method == "ROC" ||
          feature_selection_method == "roc") {
        feature_list <-
          names(training_set[, !(names(training_set) %in% non_features)])
        ...
        if (automatically_select_features == TRUE) {
          ...
          for (feature in feature_list) {
            ...
            feature_ROC <-
              roc(response = training_set[, discriminant_attribute], predictor = training_set[, feature])
            # Extract the AUC
          ...
          for (f in 1:length(feature_list)) {
            # Compute the ROC of the feature
            feature_ROC <-
              roc(response = training_set[, discriminant_attribute], predictor = training_set[, feature_list[f]])
            # Append the AUC to a vector
            feature_ROC_vector[f] <- feature_ROC$auc
          }
          # Sort the vector
          feature_ROC_vector_sorted <-
            sort(feature_ROC_vector, decreasing = TRUE)
          # Take the first N features
          features_ROC <-
            names(feature_ROC_vector_sorted)[1:features_to_select]
        }
        # Predictors
      ...
      training_set_feature_selection <-
        training_set[, predictors_feature_selection]
      # Add the non features back
      for (i in 1:length(non_features)) {
        training_set_feature_selection <-
          cbind(training_set_feature_selection, training_set[, non_features[i]])
      }
      names(training_set_feature_selection) <-
        c(as.character(predictors_feature_selection),
          non_features)
      # Return the values
      return(
        list(
          training_set_feature_selection = training_set_feature_selection,
          predictors_feature_selection = predictors_feature_selection,
          feature_selection_graphics = feature_selection_graphics,
          feature_weights = feature_weights,
          variable_importance = variable_importance,
          fs_model_performance = fs_model_performance,
          fs_model = fs_model
        )
      )
    }
    ...
    function(peaklist,
      ...
      rfe_ctrl <-
        rfeControl(
          ...
          number = k_fold_cv_control,
          saveDetails = TRUE,
          allowParallel = allow_parallelization,
          rerank = feature_reranking,
          seeds = NULL
        )
      ### Run the RFE (model tuning during feature selection or after)
      rfe_model <-
        rfe(
          x = peaklist[, !(names(peaklist) %in% non_features)],
          y = as.factor(peaklist[, discriminant_attribute]),
          sizes = subset_sizes,
          rfeControl = rfe_ctrl,
      ...
      if (selection_metric == "kappa" ||
          selection_metric == "Kappa") {
        fs_model_performance <-
          as.numeric(max(rfe_model$fit$results$Kappa, na.rm = TRUE))
        names(fs_model_performance) <- "Kappa"
      } else if (selection_metric == "accuracy" ||
                 selection_metric == "Accuracy") {
        fs_model_performance <-
          as.numeric(max(rfe_model$fit$results$Accuracy, na.rm = TRUE))
      ...
      #### Take the selected features
      peaklist_feature_selection <-
        peaklist[, predictors_feature_selection]
      ### Add the non features back
      for (i in 1:length(non_features)) {
        peaklist_feature_selection <-
          cbind(peaklist_feature_selection, peaklist[, non_features[i]])
      }
      names(peaklist_feature_selection) <-
        c(as.character(predictors_feature_selection),
          non_features)
      ...
      ### External validation
      if (!is.null(test_set) &&
          (is.matrix(test_set) || is.data.frame(test_set))) {
        ...
        predicted_classes_model <-
          predict(fs_model, newdata = test_set[, !(names(test_set) %in% non_features)])
        # Create the outcomes dataframe
        classification_results_model <-
          data.frame(
            Sample = test_set$Sample,
            Predicted = predicted_classes_model,
            True = test_set$Class
          )
        # Generate the confusion matrix to evaluate the performances (take the first class as positive if not specified)
        if (is.null(positive_class_cv) ||
            !(positive_class_cv %in% levels(as.factor(peaklist[, discriminant_attribute])))) {
          positive_class_cv <-
            levels(as.factor(peaklist[, discriminant_attribute]))[1]
        }
        model_performance_confusion_matrix <-
          confusionMatrix(
            data = predicted_classes_model,
            reference = test_set$Class,
            positive = positive_class_cv,
            mode = "everything",
        ...
        roc_curve <-
          roc(
            response = as.numeric(classification_results_model$True),
            predictor = as.numeric(classification_results_model$Predicted)
          )
        model_roc[[1]] <- roc_curve$auc
        if (generate_plots == TRUE) {
          plot(roc_curve)
          roc_legend <-
            paste("ROC area under the curve:", roc_curve$auc)
          legend(
            "bottomright",
            legend = roc_legend,
            xjust = 0.5,
      ...
      return(
        list(
          peaklist_feature_selection = peaklist_feature_selection,
          predictors_feature_selection = predictors_feature_selection,
          feature_selection_graphics = feature_selection_graphics,
          feature_weights = feature_weights,
          variable_importance = variable_importance,
          fs_model_performance = fs_model_performance,
          feature_selection_model = fs_model,
          class_list = levels(as.factor(peaklist[, discriminant_attribute])),
          pie_chart_classification = pie_chart_classification,
          model_roc = model_roc,
          model_performance_confusion_matrix = model_performance_confusion_matrix
        )
      )
    }
  ...
  embedded_rfe <<-
    function(training_set,
             features_to_select = 20,
             selection_method = "pls",
             model_tuning = c("embedded", "after", "none"),
             model_tune_grid = list(ncomp = 1:5),
             selection_metric = "Accuracy",
             cv_repeats_control = 5,
             k_fold_cv_control = 10,
             discriminant_attribute = "Class",
      ...
      if (!is.null(model_tuning) &&
          model_tuning == "embedded" && is.list(model_tune_grid)) {
        # Run the RFE
        rfe_model <-
          rfe(
            x = training_set[, !(names(training_set) %in% non_features)],
            ...
            metric = selection_metric,
            preProcess = preprocessing,
            tuneGrid = expand.grid(model_tune_grid)
          )
      } else if (is.null(model_tuning) ||
                 model_tuning == "after" ||
                 (model_tuning == "no" || model_tuning == "none")) {
      ...
      model_cv_performance_parameters <-
        extract_model_performance_parameters(
          training_set = training_set,
          model_object = fs_model,
          discriminant_attribute = discriminant_attribute,
          non_features = non_features,
          type_of_analysis = "cross validation",
          test_set = NULL
        )
      model_cv_performance_parameter_list <-
        model_cv_performance_parameters$performance_parameter_list
      cross_validation_confusion_matrix <-
        model_cv_performance_parameters$confusion_matrix
      cross_validation_confusion_matrix_df <-
        model_cv_performance_parameters$confusion_matrix_df
      # Variable importance
      ...
      if (automatically_select_features == TRUE) {
        ...
        predictors_feature_selection <-
          predictors(rfe_model$fit)[1:features_to_select]
        if (length(predictors_feature_selection[is.na(predictors_feature_selection)]) > 0) {
          predictors_feature_selection <-
            predictors(rfe_model)[1:features_to_select]
        }
      }
      ##### Plots
      if (generate_plots == TRUE) {
        ...
        try({
          ...
          model_tuning_graphics <-
            plot(rfe_model$fit, type = c("g", "o"))
        }, silent = TRUE)
      } else {
        feature_selection_graphics <- NULL
        model_tuning_graphics <- NULL
      ...
      training_set_feature_selection <-
        training_set[, predictors_feature_selection]
      colnames_training_set_feature_selection <-
        colnames(training_set_feature_selection)
      ### Add the non features back
      for (i in 1:length(non_features)) {
        training_set_feature_selection <-
          cbind(training_set_feature_selection, training_set[, non_features[i]])
      ...
      names(training_set_feature_selection) <-
        c(as.character(colnames_training_set_feature_selection),
          non_features)
      #### Tune the model after the features have been selected
      if ((!is.null(model_tuning) &&
           model_tuning == "after") &&
          (!is.null(model_tune_grid) ||
           (is.list(model_tune_grid) && length(model_tune_grid) > 0))) {
        # Make the randomness reproducible
        if (!is.null(seed)) {
          set.seed(seed)
        }
        ...
        fs_model_tuning <-
          train(
            x = training_set_feature_selection[, !(names(training_set_feature_selection) %in% non_features)],
            y = as.factor(training_set_feature_selection[, discriminant_attribute]),
            method = selection_method,
            preProcess = preprocessing,
            tuneGrid = expand.grid(model_tune_grid),
            trControl = train_ctrl,
        ...
        if (selection_metric == "kappa" ||
            selection_metric == "Kappa") {
          fs_model_performance_tuning <-
            as.numeric(max(fs_model_tuning$results$Kappa, na.rm = TRUE))
          names(fs_model_performance_tuning) <- "Kappa"
        } else if (selection_metric == "accuracy" ||
                   selection_metric == "Accuracy") {
          fs_model_performance_tuning <-
            as.numeric(max(fs_model_tuning$results$Accuracy, na.rm = TRUE))
      ...
      if (!is.null(test_set) &&
        ...
        predicted_classes_model <-
          predict(fs_model, newdata = test_set[, !(names(test_set) %in% non_features)])#, type = "raw")
        # Determine the performance parameters
        model_external_performance_parameters <-
          extract_model_performance_parameters(
            training_set = training_set_feature_selection,
            model_object = fs_model,
            discriminant_attribute = discriminant_attribute,
            non_features = non_features,
            type_of_analysis = "external validation",
            test_set = test_set
          )
        model_external_performance_parameter_list <-
          model_external_performance_parameters$performance_parameter_list
        external_validation_confusion_matrix <-
          model_external_performance_parameters$confusion_matrix
        external_validation_confusion_matrix_df <-
          model_external_performance_parameters$confusion_matrix_df
        # Create the outcomes dataframe
        classification_results_model <-
          data.frame(
            Sample = rownames(test_set),
            Predicted = predicted_classes_model,
            True = test_set[, discriminant_attribute]
          )
        ## ROC analysis
        model_roc <- list()
        roc_curve <-
        ...
        if (generate_plots == TRUE) {
          ...
          try({
            plot(roc_curve)
            roc_legend <-
              paste("ROC area under the curve:", round(roc_curve$auc, 3))
            legend(
              "bottomright",
              legend = roc_legend,
              xjust = 0.5,
      ...
      return(
        list(
          ...
          predictors_feature_selection = predictors_feature_selection,
          feature_selection_graphics = feature_selection_graphics,
          model_tuning_graphics = model_tuning_graphics,
          feature_weights = feature_weights,
          variable_importance = variable_importance,
          fs_model_performance = fs_model_performance,
          feature_selection_model = fs_model,
          ...
          pie_chart_classification = pie_chart_classification,
          model_roc = model_roc,
          external_validation_confusion_matrix = external_validation_confusion_matrix,
          external_validation_confusion_matrix_df = external_validation_confusion_matrix_df,
          cross_validation_confusion_matrix = cross_validation_confusion_matrix,
          cross_validation_confusion_matrix_df = cross_validation_confusion_matrix_df,
          model_cv_performance_parameter_list = model_cv_performance_parameter_list,
          model_external_performance_parameter_list = model_external_performance_parameter_list
        )
      )
    }
    ...
    function(training_set,
      ...
      if (try_combination_of_parameters == TRUE) {
        ...
        preprocessing_values <-
          list(NULL, "center", "scale", c("center", "scale"))
        ...
        for (p in 1:length(preprocessing_values)) {
          for (f in 1:length(feature_reranking_values)) {
            parameter_combination[[(length(parameter_combination) + 1)]] <-
              list(preprocessing = preprocessing_values[[p]],
                   feature_reranking = feature_reranking_values[f])
          }
        }
          ...
          try({
            single_rfe_model <-
              embedded_rfe(
                ...
                model_tune_grid = model_tune_grid,
                ...
                preprocessing = parameter_combination[[comb]]$preprocessing,
                allow_parallelization = allow_parallelization,
                feature_reranking = parameter_combination[[comb]]$feature_reranking,
                test_set = test_set,
                positive_class_cv = positive_class_cv
              )
            ### Check (and store) the performance values and the model
            if (is.null(best_model_performance) ||
                single_rfe_model$fs_model_performance > best_model_performance) {
  ...
  model_ensemble_embedded_fs <<-
    function(training_set,
             ...
             outcome_list = c("benign", "malignant"),
             progress_bar = NULL,
             test_set = NULL) {
      ...
      pls_model_cross_validation_confusion_matrix <-
        pls_model_rfe$cross_validation_confusion_matrix
      pls_model_cross_validation_confusion_matrix_df <-
        pls_model_rfe$cross_validation_confusion_matrix_df
      pls_model_cv_performance_parameter_list <-
        pls_model_rfe$model_cv_performance_parameter_list
      pls_model_external_validation_confusion_matrix <-
        pls_model_rfe$external_validation_confusion_matrix
      pls_model_external_validation_confusion_matrix_df <-
        pls_model_rfe$external_validation_confusion_matrix_df
      pls_model_external_performance_parameter_list <-
        pls_model_rfe$model_external_performance_parameter_list
      cat("\nPartial Least Squares\n")
      ...
      if (!is.null(progress_bar) && progress_bar == "tcltk") {
        setTkProgressBar(
          ...
          value = 0.10,
          title = NULL,
          label = "RBF Support Vector Machines"
        )
      } else if (!is.null(progress_bar) && progress_bar == "txt") {
        setTxtProgressBar(
          fs_progress_bar,
      ...
      svmRadial_model_rfe <-
        automated_embedded_rfe(
          training_set = training_set,
          features_to_select = features_to_select,
          selection_method = "svmRadial",
          model_tuning = model_tuning,
          model_tune_grid = list(sigma = 10 ^ (-3:3), C = 10 ^ (-3:3)),
          selection_metric = selection_metric,
          cv_repeats_control = cv_repeats_control,
          k_fold_cv_control = k_fold_cv_control,
          discriminant_attribute = discriminant_attribute,
      ...
      svmRadial_model_features <-
        svmRadial_model_rfe$predictors_feature_selection
      svmRadial_model_class_list <- svmRadial_model_rfe$class_list
      svmRadial_model_ID <- "svm"
      svmRadial_model_performance <-
        svmRadial_model_rfe$fs_model_performance
      svmRadial_model_cross_validation_confusion_matrix <-
        svmRadial_model_rfe$cross_validation_confusion_matrix
      svmRadial_model_cross_validation_confusion_matrix_df <-
        svmRadial_model_rfe$cross_validation_confusion_matrix_df
      svmRadial_model_cv_performance_parameter_list <-
        svmRadial_model_rfe$model_cv_performance_parameter_list
      svmRadial_model_external_validation_confusion_matrix <-
        svmRadial_model_rfe$external_validation_confusion_matrix
      svmRadial_model_external_validation_confusion_matrix_df <-
        svmRadial_model_rfe$external_validation_confusion_matrix_df
      svmRadial_model_external_performance_parameter_list <-
        svmRadial_model_rfe$model_external_performance_parameter_list
      ...
      if (!is.null(progress_bar) && progress_bar == "tcltk") {
        setTkProgressBar(
          ...
          value = 0.20,
          title = NULL,
          label = "Polynomial Support Vector Machines"
        )
      } else if (!is.null(progress_bar) && progress_bar == "txt") {
        setTxtProgressBar(
          fs_progress_bar,
      ...
      svmPoly_model_rfe <-
        automated_embedded_rfe(
          training_set = training_set,
          features_to_select = features_to_select,
          selection_method = "svmPoly",
          model_tuning = model_tuning,
          model_tune_grid = list(
            C = 10 ^ (-3:3),
            degree = 1:5,
            scale = 1
          ),
      ...
      svmPoly_model_features <-
        svmPoly_model_rfe$predictors_feature_selection
      svmPoly_model_class_list <- svmPoly_model_rfe$class_list
      svmPoly_model_ID <- "svm"
      svmPoly_model_performance <-
        svmPoly_model_rfe$fs_model_performance
      svmPoly_model_cross_validation_confusion_matrix <-
        svmPoly_model_rfe$cross_validation_confusion_matrix
      svmPoly_model_cross_validation_confusion_matrix_df <-
        svmPoly_model_rfe$cross_validation_confusion_matrix_df
      svmPoly_model_cv_performance_parameter_list <-
        svmPoly_model_rfe$model_cv_performance_parameter_list
      svmPoly_model_external_validation_confusion_matrix <-
        svmPoly_model_rfe$external_validation_confusion_matrix
      svmPoly_model_external_validation_confusion_matrix_df <-
        svmPoly_model_rfe$external_validation_confusion_matrix_df
      svmPoly_model_external_performance_parameter_list <-
        svmPoly_model_rfe$model_external_performance_parameter_list
      ...
      svmLinear_model_rfe <-
        automated_embedded_rfe(
          training_set = training_set,
          features_to_select = features_to_select,
          selection_method = "svmLinear",
          model_tuning = model_tuning,
          model_tune_grid = list(C = 10 ^ (-3:3)),
          selection_metric = selection_metric,
          cv_repeats_control = cv_repeats_control,
          k_fold_cv_control = k_fold_cv_control,
          discriminant_attribute = discriminant_attribute,
      ...
      svmLinear_model_features <-
        svmLinear_model_rfe$predictors_feature_selection
      svmLinear_model_class_list <- svmLinear_model_rfe$class_list
      svmLinear_model_ID <- "svm"
      svmLinear_model_performance <-
        svmLinear_model_rfe$fs_model_performance
      svmLinear_model_cross_validation_confusion_matrix <-
        svmLinear_model_rfe$cross_validation_confusion_matrix
      svmLinear_model_cross_validation_confusion_matrix_df <-
        svmLinear_model_rfe$cross_validation_confusion_matrix_df
      svmLinear_model_cv_performance_parameter_list <-
        svmLinear_model_rfe$model_cv_performance_parameter_list
      svmLinear_model_external_validation_confusion_matrix <-
        svmLinear_model_rfe$external_validation_confusion_matrix
      svmLinear_model_external_validation_confusion_matrix_df <-
        svmLinear_model_rfe$external_validation_confusion_matrix_df
      svmLinear_model_external_performance_parameter_list <-
        svmLinear_model_rfe$model_external_performance_parameter_list
      ...
      rf_model_rfe <-
        automated_embedded_rfe(
          ...
          selection_method = "rf",
          model_tuning = model_tuning,
          model_tune_grid = list(mtry = seq(1, 5, by = 1)),
          selection_metric = selection_metric,
          cv_repeats_control = cv_repeats_control,
          k_fold_cv_control = k_fold_cv_control,
          discriminant_attribute = discriminant_attribute,
      ...
      rf_model_cross_validation_confusion_matrix <-
        rf_model_rfe$cross_validation_confusion_matrix
      rf_model_cross_validation_confusion_matrix_df <-
        rf_model_rfe$cross_validation_confusion_matrix_df
      rf_model_cv_performance_parameter_list <-
        rf_model_rfe$model_cv_performance_parameter_list
      rf_model_external_validation_confusion_matrix <-
        rf_model_rfe$external_validation_confusion_matrix
      rf_model_external_validation_confusion_matrix_df <-
        rf_model_rfe$external_validation_confusion_matrix_df
      rf_model_external_performance_parameter_list <-
        rf_model_rfe$model_external_performance_parameter_list
      cat("\nRandom Forest\n")
      ...
      nbc_model_cross_validation_confusion_matrix <-
        nbc_model_rfe$cross_validation_confusion_matrix
      nbc_model_cross_validation_confusion_matrix_df <-
        nbc_model_rfe$cross_validation_confusion_matrix_df
      nbc_model_cv_performance_parameter_list <-
        nbc_model_rfe$model_cv_performance_parameter_list
      nbc_model_external_validation_confusion_matrix <-
        nbc_model_rfe$external_validation_confusion_matrix
      nbc_model_external_validation_confusion_matrix_df <-
        nbc_model_rfe$external_validation_confusion_matrix_df
      nbc_model_external_performance_parameter_list <-
        nbc_model_rfe$model_external_performance_parameter_list
      cat("\nNaive Bayes Classifier\n")
      ...
      knn_model_rfe <-
        automated_embedded_rfe(
          ...
          model_tuning = model_tuning,
          model_tune_grid = list(k = seq(3, 15, by = 1)),
          selection_metric = selection_metric,
          cv_repeats_control = cv_repeats_control,
          k_fold_cv_control = k_fold_cv_control,
          discriminant_attribute = discriminant_attribute,
      ...
      knn_model_cross_validation_confusion_matrix <-
        knn_model_rfe$cross_validation_confusion_matrix
      knn_model_cross_validation_confusion_matrix_df <-
        knn_model_rfe$cross_validation_confusion_matrix_df
      knn_model_cv_performance_parameter_list <-
        knn_model_rfe$model_cv_performance_parameter_list
      knn_model_external_validation_confusion_matrix <-
        knn_model_rfe$external_validation_confusion_matrix
      knn_model_external_validation_confusion_matrix_df <-
        knn_model_rfe$external_validation_confusion_matrix_df
      knn_model_external_performance_parameter_list <-
        knn_model_rfe$model_external_performance_parameter_list
      cat("\nk-Nearest Neighbor\n")
      ...
      PLS_model_list <-
        list(
          model = pls_model,
          class_list = pls_model_class_list,
          outcome_list = outcome_list,
          features_model = pls_model_features,
          model_ID = pls_model_ID,
          model_performance = pls_model_performance,
          cross_validation_confusion_matrix = pls_model_cross_validation_confusion_matrix,
          cross_validation_confusion_matrix_df = pls_model_cross_validation_confusion_matrix_df,
          model_cv_performance_parameter_list = pls_model_cv_performance_parameter_list,
          external_validation_confusion_matrix = pls_model_external_validation_confusion_matrix,
          external_validation_confusion_matrix_df = pls_model_external_validation_confusion_matrix_df,
          model_external_performance_parameter_list = pls_model_external_performance_parameter_list
        )
      # Support Vector Machine (with Radial Basis Kernel function)
      RSVM_model_list <-
        list(
          model = svmRadial_model,
          class_list = svmRadial_model_class_list,
          outcome_list = outcome_list,
          features_model = svmRadial_model_features,
          model_ID = svmRadial_model_ID,
          model_performance = svmRadial_model_performance,
          cross_validation_confusion_matrix = svmRadial_model_cross_validation_confusion_matrix,
          cross_validation_confusion_matrix_df = svmRadial_model_cross_validation_confusion_matrix_df,
          ...
          external_validation_confusion_matrix_df = svmRadial_model_external_validation_confusion_matrix_df,
          model_external_performance_parameter_list = svmRadial_model_external_performance_parameter_list
        )
      # Support Vector Machine (with Polynomial Kernel function)
      PSVM_model_list <-
        list(
          model = svmPoly_model,
          class_list = svmPoly_model_class_list,
          outcome_list = outcome_list,
          features_model = svmPoly_model_features,
          model_ID = svmPoly_model_ID,
          model_performance = svmPoly_model_performance,
          cross_validation_confusion_matrix = svmPoly_model_cross_validation_confusion_matrix,
          cross_validation_confusion_matrix_df = svmPoly_model_cross_validation_confusion_matrix_df,
          model_cv_performance_parameter_list = svmPoly_model_cv_performance_parameter_list,
          external_validation_confusion_matrix = svmPoly_model_external_validation_confusion_matrix,
          external_validation_confusion_matrix_df = svmPoly_model_external_validation_confusion_matrix_df,
          model_external_performance_parameter_list = svmPoly_model_external_performance_parameter_list
        )
      # Support Vector Machine (with Linear Kernel function)
      LSVM_model_list <-
        list(
          model = svmLinear_model,
          class_list = svmLinear_model_class_list,
          outcome_list = outcome_list,
          features_model = svmLinear_model_features,
          model_ID = svmLinear_model_ID,
          model_performance = svmLinear_model_performance,
          cross_validation_confusion_matrix = svmLinear_model_cross_validation_confusion_matrix,
          cross_validation_confusion_matrix_df = svmLinear_model_cross_validation_confusion_matrix_df,
          model_cv_performance_parameter_list = svmLinear_model_cv_performance_parameter_list,
          external_validation_confusion_matrix = svmLinear_model_external_validation_confusion_matrix,
          external_validation_confusion_matrix_df = svmLinear_model_external_validation_confusion_matrix_df,
      ...
      RF_model_list <-
        list(
          model = rf_model,
          class_list = rf_model_class_list,
          outcome_list = outcome_list,
          features_model = rf_model_features,
          model_ID = rf_model_ID,
          model_performance = rf_model_performance,
          cross_validation_confusion_matrix = rf_model_cross_validation_confusion_matrix,
          cross_validation_confusion_matrix_df = rf_model_cross_validation_confusion_matrix_df,
          model_cv_performance_parameter_list = rf_model_cv_performance_parameter_list,
          external_validation_confusion_matrix = rf_model_external_validation_confusion_matrix,
          external_validation_confusion_matrix_df = rf_model_external_validation_confusion_matrix_df,
          model_external_performance_parameter_list = rf_model_external_performance_parameter_list
        )
      # Naive Bayes Classifier
      NBC_model_list <-
        list(
          model = nbc_model,
          class_list = nbc_model_class_list,
          outcome_list = outcome_list,
          features_model = nbc_model_features,
          model_ID = nbc_model_ID,
          model_performance = nbc_model_performance,
          cross_validation_confusion_matrix = nbc_model_cross_validation_confusion_matrix,
          cross_validation_confusion_matrix_df = nbc_model_cross_validation_confusion_matrix_df,
          model_cv_performance_parameter_list = nbc_model_cv_performance_parameter_list,
          external_validation_confusion_matrix = nbc_model_external_validation_confusion_matrix,
      ...
      KNN_model_list <-
        list(
          model = knn_model,
          class_list = knn_model_class_list,
          outcome_list = outcome_list,
          features_model = knn_model_features,
          model_ID = knn_model_ID,
          model_performance = knn_model_performance,
          cross_validation_confusion_matrix = knn_model_cross_validation_confusion_matrix,
          cross_validation_confusion_matrix_df = knn_model_cross_validation_confusion_matrix_df,
          model_cv_performance_parameter_list = knn_model_cv_performance_parameter_list,
          external_validation_confusion_matrix = knn_model_external_validation_confusion_matrix,
          external_validation_confusion_matrix_df = knn_model_external_validation_confusion_matrix_df,
          model_external_performance_parameter_list = knn_model_external_performance_parameter_list
        )
      # Neural Network
      ...
      model_list <-
        list(
          "SVM Radial Basis" = RSVM_model_list,
          "SVM Polynomial" = PSVM_model_list,
          "SVM Linear" = LSVM_model_list,
          "Partial Least Squares" = PLS_model_list,
          "Random Forest" = RF_model_list,
          "Naive Bayes Classifier" = NBC_model_list,
          "k-Nearest Neighbor" = KNN_model_list
        )#, "Neural Network" = NNET_model_list)
      ### Build the final feature vector
      feature_list <-
        extract_feature_list_from_model_list(filepath_R = model_list, features_to_return = features_to_select)
      ### Yield the peaklist matrix with only the common features
      # Extract the common features
      common_features_list <-
        extract_common_features_from_model_list(filepath_R = model_list, features_to_return = common_features_to_select)
      ...
      if (length(common_features_list) > 0) {
        ...
        training_set_common_features <-
          training_set[, c(common_features_list, non_features)]
      } else {
      ...
      if (length(common_features_list) > 0) {
        for (cf in 1:length(common_features_list)) {
          if (base::startsWith(common_features_list[cf], "X")) {
            common_features_list[cf] <-
              as.numeric(unlist(strsplit(common_features_list[cf], "X"))[2])
          }
        }
        common_features_matrix <-
          as.matrix(cbind(common_features_list))
      ...
      model_performance_matrix <-
        extract_performance_matrix_from_model_list(filepath_R = model_list)
      ### Models performance parameter list
      model_performance_parameter_list <-
        list("cv" = list(), "external" = list())
      for (m in 1:length(model_list)) {
        model_performance_parameter_list[["cv"]][[names(model_list)[m]]] <-
          model_list[[m]]$model_cv_performance_parameter_list
        model_performance_parameter_list[["external"]][[names(model_list)[m]]] <-
          model_list[[m]]$model_external_performance_parameter_list
      }
      # Progress bar
      ...
      return(
        list(
          ...
          common_features_list = common_features_list,
          training_set_common_features = training_set_common_features,
          common_features_matrix = common_features_matrix,
          model_performance_matrix = model_performance_matrix,
          model_performance_parameter_list = model_performance_parameter_list
        )
      )
    }
  ...
  extract_model_performance_parameters <<-
    function(training_set,
      ...
      class_list <-
        levels(as.factor(training_set[, discriminant_attribute]))
      ##### CROSS-VALIDATION
      if (type_of_analysis == "cross validation" ||
          type_of_analysis == "cv" || type_of_analysis == "CV") {
        ...
        for (cl in 1:length(class_list)) {
          ...
          negative_class_cv <-
            class_list[class_list != positive_class_cv]
          ...
          if (is.null(confusion_matrix_df)) {
            for (cl in 1:ncol(confusion_matrix$table)) {
              if (is.null(confusion_matrix_df)) {
                ...
                confusion_matrix_df <-
                  as.matrix(cbind(confusion_matrix_df, confusion_matrix$table[, cl]))
              }
            }
            colnames(confusion_matrix_df) <-
              colnames(confusion_matrix$table)
            rownames(confusion_matrix_df) <-
              rownames(confusion_matrix$table)
          ...
          performance_parameter_vector[["class proportion"]] <-
            nrow(training_set[training_set[, discriminant_attribute] == positive_class_cv,]) / nrow(training_set)
          true_positives <-
            confusion_matrix_df[positive_class_cv, positive_class_cv]
          true_negatives <-
            confusion_matrix_df[negative_class_cv, negative_class_cv]
          false_positives <-
            confusion_matrix_df[positive_class_cv, negative_class_cv]
          false_negatives <-
            confusion_matrix_df[negative_class_cv, positive_class_cv]
          performance_parameter_vector[["sensitivity"]] <-
            true_positives / (true_positives + false_negatives)
          performance_parameter_vector[["specificity"]] <-
            true_negatives / (true_negatives + false_positives)
          performance_parameter_vector[["TPR"]] <-
            performance_parameter_vector[["sensitivity"]]
          performance_parameter_vector[["TNR"]] <-
            performance_parameter_vector[["specificity"]]
          performance_parameter_vector[["FPR"]] <-
            false_positives / (true_negatives + false_positives)
          performance_parameter_vector[["FNR"]] <-
            false_negatives / (true_positives + false_negatives)
          performance_parameter_vector[["accuracy"]] <-
            (true_positives + true_negatives) / (true_positives + true_negatives + false_positives + false_negatives)
          performance_parameter_vector[["PPV"]] <-
            true_positives / (true_positives + false_positives)
          performance_parameter_vector[["NPV"]] <-
            true_negatives / (true_negatives + false_negatives)
          performance_parameter_vector[["FDR"]] <-
            1 - performance_parameter_vector[["PPV"]]
          performance_parameter_vector[["FOR"]] <-
            1 - performance_parameter_vector[["NPV"]]
          performance_parameter_vector[["LR+"]] <-
            performance_parameter_vector[["TPR"]] / performance_parameter_vector[["FPR"]]
          performance_parameter_vector[["LR-"]] <-
            performance_parameter_vector[["FNR"]] / performance_parameter_vector[["TNR"]]
          performance_parameter_vector[["DOR"]] <-
            performance_parameter_vector[["LR+"]] / performance_parameter_vector[["LR-"]]
          performance_parameter_vector[["balanced accuracy"]] <-
            (performance_parameter_vector[["sensitivity"]] + performance_parameter_vector[["specificity"]]) / 2
          performance_parameter_vector[["recall"]] <-
            performance_parameter_vector[["sensitivity"]]
          performance_parameter_vector[["precision"]] <-
            performance_parameter_vector[["PPV"]]
          # Store the vector in the list, named according to the positive class of the CV
          performance_parameter_list[[positive_class_cv]] <-
            performance_parameter_vector
        }
        ### Return
        return(
          list(
            performance_parameter_list = performance_parameter_list,
      ...
      (!is.null(test_set) &&
       (is.matrix(test_set) || is.data.frame(test_set)))) {
        ...
        predicted_classes_model <-
          as.character(predict(model_object, newdata = test_set[, !(names(test_set) %in% non_features)]))#, type = "raw"))
        ### Determine the performance parameter values for each class (at each iteration, one class will be considered as the positive class for the confusion matrix)
        for (cl in 1:length(class_list)) {
          # One is the positive class
          positive_class_cv <- class_list[cl]
          ...
          if (is.null(confusion_matrix)) {
            confusion_matrix <-
              confusionMatrix(
                data = predicted_classes_model,
                reference = test_set[, discriminant_attribute],
                positive = positive_class_cv,
                dnn = c("Predicted", "Actual"),
                mode = "everything"
              )
  ...
  single_model_training_and_tuning <<-
    function(training_set_feature_selection,
             non_features = c("Sample", "Class"),
             discriminant_attribute = "Class",
             preprocessing = c("center", "scale"),
             selection_method = "pls",
      ...
      if (selection_metric == "kappa" ||
          selection_metric == "Kappa") {
        fs_model_performance <-
          as.numeric(max(fs_model$results$Kappa, na.rm = TRUE))
        names(fs_model_performance) <- "Kappa"
      } else if (selection_metric == "accuracy" ||
                 selection_metric == "Accuracy") {
        fs_model_performance <-
          as.numeric(max(fs_model$results$Accuracy, na.rm = TRUE))
        names(fs_model_performance) <- "Accuracy"
      }
      ### Plots
      if (generate_plots == TRUE) {
      ...
      if (!is.null(test_set) &&
        ...
        # Generate the confusion matrix to evaluate the performances (take the first class as positive if not specified)
        if (is.null(positive_class_cv) ||
            !(positive_class_cv %in% levels(as.factor(training_set_feature_selection[, discriminant_attribute])))) {
          positive_class_cv <-
            levels(as.factor(training_set_feature_selection[, discriminant_attribute]))[1]
        }
        model_performance_confusion_matrix <-
          confusionMatrix(
            data = predicted_classes_model,
      ...
      return(
        list(
          fs_model = fs_model,
          fs_model_performance = fs_model_performance,
          class_list = levels(as.factor(training_set_feature_selection[, discriminant_attribute])),
          feature_list = colnames(training_set_feature_selection[, !(names(training_set_feature_selection) %in% non_features)]),
          model_performance_confusion_matrix = model_performance_confusion_matrix,
          pie_chart_classification = pie_chart_classification,
          model_roc = model_roc
        )
    ...
    function(training_set_feature_selection,
      ...
      if (try_combination_of_parameters == TRUE) {
        ...
        for (comb in 1:length(parameter_combination)) {
          ...
          ### Check (and store) the performance values and the model
          if (is.null(best_model_performance) ||
              single_model$fs_model_performance > best_model_performance) {
  ...
  # The function returns a list of models (with the model object, the model ID, the performances, the outcome list, the class list and the features), a matrix listing the cross-validation performances for each model and the feature list (common and for each model).
  model_ensemble_training_and_tuning <<-
    function(training_set_feature_selection,
             non_features = c("Sample", "Class"),
             discriminant_attribute = "Class",
             preprocessing = c("center", "scale"),
      ...
      pls_model_ID <- "pls"
      pls_model_performance <-
        pls_model_training_and_tuning$fs_model_performance
      cat("\nPartial Least Squares\n")
      cat(pls_model_performance)
      # Progress bar
      if (!is.null(progress_bar) && progress_bar == "tcltk") {
      ...
      svmRadial_model_training_and_tuning <-
        automated_single_model_training_and_tuning(
          ...
          preprocessing = preprocessing,
          selection_method = "svmRadial",
          model_tune_grid = list(sigma = 10 ^ (-5:5), C = 10 ^ (-5:5)),
          selection_metric = selection_metric,
          cv_repeats_control = cv_repeats_control,
          k_fold_cv_control = k_fold_cv_control,
          seed = seed,
      ...
      svmRadial_model_features <-
        svmRadial_model_training_and_tuning$feature_list
      svmRadial_model_class_list <-
        svmRadial_model_training_and_tuning$class_list
      svmRadial_model_ID <- "svm"
      svmRadial_model_performance <-
        svmRadial_model_training_and_tuning$fs_model_performance
      cat("\nSupport Vector Machines (with Radial Basis Kernel function)\n")
      cat(svmRadial_model_performance)
      # Progress bar
      if (!is.null(progress_bar) && progress_bar == "tcltk") {
      ...
      svmPoly_model_features <-
        svmPoly_model_training_and_tuning$feature_list
      svmPoly_model_class_list <-
        svmPoly_model_training_and_tuning$class_list
      svmPoly_model_ID <- "svm"
      svmPoly_model_performance <-
        svmPoly_model_training_and_tuning$fs_model_performance
      cat("\nSupport Vector Machines (with Polynomial Kernel function)\n")
      cat(svmPoly_model_performance)
      # Progress bar
      if (!is.null(progress_bar) && progress_bar == "tcltk") {
      ...
      rf_model_performance <-
        rf_model_training_and_tuning$fs_model_performance
      cat("Random Forest")
      ...
      nbc_model_ID <- "nbc"
      nbc_model_performance <-
        nbc_model_training_and_tuning$fs_model_performance
      ...
      knn_model_training_and_tuning <-
        automated_single_model_training_and_tuning(
          ...
          preprocessing = preprocessing,
          selection_method = "knn",
          model_tune_grid = list(k = seq(1, 15, by = 1)),
          selection_metric = selection_metric,
          cv_repeats_control = cv_repeats_control,
          k_fold_cv_control = k_fold_cv_control,
          seed = seed,
      ...
      knn_model_performance <-
        knn_model_training_and_tuning$fs_model_performance
      ...
      # Neural Network
      nnet_model_training_and_tuning <-
        automated_single_model_training_and_tuning(
          training_set_feature_selection,
          non_features = non_features,
          discriminant_attribute = discriminant_attribute,
          preprocessing = preprocessing,
          selection_method = "nnet",
          model_tune_grid = list(size = seq(1, 5, by = 1), decay = seq(0, 2, by = 1)),
          selection_metric = selection_metric,
          cv_repeats_control = cv_repeats_control,
          k_fold_cv_control = k_fold_cv_control,
          seed = seed,
      ...
      nnet_model_features <-
        nnet_model_training_and_tuning$feature_list
      nnet_model_class_list <-
        nnet_model_training_and_tuning$class_list
      nnet_model_ID <- "nnet"
      nnet_model_performance <-
        nnet_model_training_and_tuning$fs_model_performance
      cat("Neural Network")
      ...
      NNET_model_list <-
        list(
          model = nnet_model,
          class_list = nnet_model_class_list,
          outcome_list = outcome_list,
          features_model = nnet_model_features,
          model_ID = nnet_model_ID,
          model_performance = nnet_model_performance
        )
      # Progress bar
      if (!is.null(progress_bar) && progress_bar == "tcltk") {
        setTkProgressBar(
      ...
      ### Build the final feature vector
      feature_list <-
        extract_feature_list_from_model_list(filepath_R = model_list, features_to_return = 0)
      ### Build the matrix with the model performances
      model_performance_matrix <-
        extract_performance_matrix_from_model_list(filepath_R = model_list)
      # Progress bar
  ...
  spectral_typer_score_hierarchical_distance <<-
    function(spectra_reference,
             spectra_test,
             class_list_library = NULL,
             peaks_filtering_percentage_threshold = 5,
             low_intensity_percentage_threshold = 0,
             low_intensity_threshold_method = "element-wise",
             tof_mode = "linear",
             spectra_format = "fid",
      ...
      if (!is.null(spectral_alignment_algorithm)) {
        spectra_all <-
          align_spectra(
            ...
            spectral_alignment_reference = spectral_alignment_reference,
            tof_mode = tof_mode,
            deisotope_peaklist = peak_deisotoping,
            envelope_peaklist = peak_enveloping,
            tolerance_ppm = tolerance_ppm
          )
        spectra_reference <- spectra_all[1:reference_size]
        spectra_test <-
          spectra_all[(reference_size + 1):length(spectra_all)]
      ...
      peaks_reference <-
        peak_picking(
          ...
          deisotope_peaklist = peak_deisotoping,
          envelope_peaklist = peak_enveloping,
          signals_to_take = signals_to_take
        )
      peaks_test <-
        peak_picking(
          spectra = spectra_test,
          peak_picking_algorithm = peak_picking_algorithm,
          tof_mode = tof_mode,
          SNR = peak_picking_SNR,
          allow_parallelization = allow_parallelization,
      ...
      peaks_all <-
        align_and_filter_peaks(
          peaks_all,
          tof_mode = tof_mode,
          peak_filtering_frequency_threshold_percent = peaks_filtering_percentage_threshold,
          low_intensity_peak_removal_threshold_percent = low_intensity_percentage_threshold,
          low_intensity_peak_removal_threshold_method = low_intensity_threshold_method,
          tolerance_ppm = tolerance_ppm
        )
      # Restore the lists
      ...
      peaks_test <-
        replace_sample_name(peaks_test, spectra_format = spectra_format)
      peaks_reference <-
        replace_class_name(peaks_reference,
                           class_list = class_list_library,
                           spectra_format = spectra_format)
      ...
      if (is.null(names(peaks_test))) {
        ...
        for (s in 1:number_of_samples) {
          sample_vector <-
            append(sample_vector, peaks_test[[s]]@metaData$file[1])
        }
      } else {
      ...
      if (is.null(names(peaks_reference))) {
        ...
        for (s in 1:reference_size) {
          reference_vector <-
            append(reference_vector, peaks_reference[[s]]@metaData$file[1])
        }
      } else {
      ...
      for (sp in 1:number_of_samples) {
        spectra_path_vector <-
          append(spectra_path_vector, spectra_test[[sp]]@metaData$file[1])
      }
      ...
      # Compute the hca
      distance_matrix <-
        dist(peaklist_matrix[, 1:(ncol(peaklist_matrix) - 1)], method = hierarchical_distance_method)
      ...
      hca_dendrogram <-
        ggdendrogram(
          ...
          rotate = TRUE,
          theme_dendro = TRUE
        )#, main = "Hierarchical clustering analysis - Spectral Typer", xlab = "Samples", ylab = "Tree height")
      ...
      distance_matrix <-
        as.matrix(distance_matrix[(reference_size + 1):nrow(distance_matrix), 1:reference_size])
      ### Normalize the euclidean distances (per sample)
      if (normalize_distances == TRUE &&
          nrow(distance_matrix) > 1 && ncol(distance_matrix) > 1) {
        ...
        if (normalization_method == "sum") {
          ...
          for (r in 1:nrow(distance_matrix)) {
            distance_matrix[r, ] <-
              distance_matrix[r, ] / max(distance_matrix[r, ], na.rm = TRUE)
          }
          ...
          for (r in 1:nrow(distance_matrix)) {
            distance_matrix[r, ] <-
              distance_matrix[r, ] / sqrt(sum(distance_matrix[r, ], na.rm = TRUE) ^ 2)
          }
          # Multiply everything by 10, to have more readable results
          distance_matrix <- distance_matrix * 10
          # The classification is made by comparing the single sample spectrum with the spectrum of the database class (the distance is displayed in the distance matrix): the closer the better
  ...
  spectral_typer_score_correlation_matrix <<-
    function(spectra_reference,
             spectra_test,
             filepath_reference,
             filepath_test,
             ...
             correlation_mode = c("common signals", "all signals"),
      ...
      if (!is.numeric(score_threshold_values) ||
          (is.numeric(score_threshold_values) &&
           length(score_threshold_values) != 2)) {
        score_threshold_values <- c(1.7, 2)
      } else if (is.numeric(score_threshold_values) &&
                 length(score_threshold_values) == 2) {
      ...
      spectra_test <-
        replace_sample_name(spectra_test, spectra_format = spectra_format)
      spectra_reference <-
        replace_class_name(
          spectra_reference,
          class_list = class_list_library,
      ...
      if (is.null(names(spectra_test))) {
        ...
        for (s in 1:number_of_samples) {
          sample_vector <-
            append(sample_vector, spectra_test[[s]]@metaData$file[1])
        }
      } else {
      ...
      if (is.null(names(spectra_reference))) {
        ...
        for (s in 1:reference_size) {
          reference_vector <-
            append(reference_vector, spectra_reference[[s]]@metaData$file[1])
        }
      } else {
      ...
      for (spl in 1:number_of_samples) {
        ...
        reference_sample_list[[spl]] [["spectra_reference"]] <-
          spectra_reference
        reference_sample_list[[spl]] [["spectrum_sample"]] <-
          spectra_test[[spl]]
        reference_sample_list[[spl]] [["reference_vector"]] <-
          reference_vector
        reference_sample_list[[spl]] [["sample_ID"]] <-
          sample_vector[spl]
      }
      names(reference_sample_list) <- names(spectra_test)
      ##### Define the function for lapply
      # x = each element of the reference_sample_list
      comparison_sample_db_subfunction_correlation <-
        function(x, modality) {
          ...
          matching_signals_matrix <-
            matrix(0, nrow = 1, ncol = reference_size)
          ...
          number_of_signals_reference_matrix <-
            matrix(0, nrow = 1, ncol = reference_size)
          rownames(number_of_signals_reference_matrix) <- x$sample_ID
          colnames(number_of_signals_reference_matrix) <-
            reference_vector
          # Number of common signals for sample-reference pairs
          number_of_signals_reference_samples_matrix <-
            matrix(0, nrow = 1, ncol = reference_size)
          rownames(number_of_signals_reference_samples_matrix) <-
            x$sample_ID
          colnames(number_of_signals_reference_samples_matrix) <-
            reference_vector
          # Common signals for sample-reference pairs
          common_signals_reference_samples_matrix <-
            matrix(0, nrow = 1, ncol = reference_size)
          rownames(common_signals_reference_samples_matrix) <-
            x$sample_ID
          colnames(common_signals_reference_samples_matrix) <-
            reference_vector
          ...
          most_influencing_signals_reference_samples_matrix <-
            matrix(0, nrow = 1, ncol = reference_size)
          rownames(most_influencing_signals_reference_samples_matrix) <-
            x$sample_ID
          colnames(most_influencing_signals_reference_samples_matrix) <-
            reference_vector
          # FIT matrix
          ...
          intensity_correlation_matrix <-
            matrix(0, ncol = reference_size, nrow = 1)
          ...
          for (db in 1:reference_size) {
            ...
            ## Align spectra
            spectra_all <- list()
            spectra_all[["spectrum_reference_x"]] <-
              spectrum_reference_x
            ...
            number_of_signals_reference_matrix[1, db] <-
              number_of_signals_database
            ## COUNTER 0 - MATCHING SIGNALS
            # Create a counter, symmetrical to the database Peaklist
            # For each peaklist in the Library
            if (length(peaks_sample_x@mass) > 0 &&
                length(peaks_reference_x@mass) > 0) {
              matching_signals_number <-
                length(intersect(peaks_sample_x@mass, peaks_reference_x@mass))
              matching_signals <-
                intersect(peaks_sample_x@mass, peaks_reference_x@mass)
            } else if (length(peaks_sample_x@mass) == 0 ||
                       length(peaks_reference_x@mass) == 0) {
            ...
            common_signals_reference_samples_matrix[1, db] <-
              as.character(paste(matching_signals, collapse = " , "))
            ## COUNTER 1 - FIT
            if (length(peaks_sample_x@mass) > 0 &&
                length(peaks_reference_x@mass) > 0) {
            ...
            if (length(peaks_sample_x@mass) > 0 &&
                length(peaks_reference_x@mass) > 0) {
              retrofit_sample <-
                matching_signals_number / length(peaks_reference_x@mass)
            } else {
              retrofit_sample <- 0
            }
            # Append this row to the global matrix
            ...
            if (modality == "common signals") {
              ...
              if (length(matching_signals) > 0) {
                common_peaks_reference_x_intensity <-
                  peaks_reference_x@intensity[peaks_reference_x@mass %in% matching_signals]
                names(common_peaks_reference_x_intensity) <-
                  matching_signals
                common_peaks_sample_x_intensity <-
                  peaks_sample_x@intensity[peaks_sample_x@mass %in% matching_signals]
                names(common_peaks_sample_x_intensity) <-
                  matching_signals
                ...
                if (intensity_correction_coefficient != 0 &&
                  ...
                  weights_vector <-
                    c(rep(
                      intensity_correction_coefficient,
                      length(matching_signals)
                    ))
                  intensity_correlation_sample <-
                    wtd.cors(x = common_peaks_sample_x_intensity,
                             y = common_peaks_reference_x_intensity,
                             weight = weights_vector)
                } else if (intensity_correction_coefficient == 1 ||
                           (
                             intensity_correction_coefficient != 0 &&
                             intensity_correction_coefficient != 1 &&
                             correlation_method != "pearson"
                           )) {
                  if (length(matching_signals) >= 3) {
                    correlation_sample <-
                      cor.test(x = common_peaks_sample_x_intensity,
                               y = common_peaks_reference_x_intensity,
                               method = correlation_method)
                    intensity_correlation_sample <-
                      correlation_sample$estimate
                    # pvalue
                    pvalue <- correlation_sample$p.value
                    pvalue_replacement_function <-
                      function(x, number_of_digits) {
                        if (is.na(x)) {
                          x <- "Not available"
                        } else if (x < 0.00001) {
                          x <- "< 0.00001"
                    ...
                    pvalue <-
                      pvalue_replacement_function(pvalue, number_of_digits = 6)
                    # Append this row to the global matrix
                ...
                intensity_correlation_sample <-
                  abs(intensity_correlation_sample)
                if (is.na(intensity_correlation_sample)) {
                  intensity_correlation_sample <- 0
                }
                # Append this row to the global matrix
                intensity_correlation_matrix[1, db] <-
                  intensity_correlation_sample
                ## COUNTER 4 - REGRESSION CURVE
                linear_regression <-
                  lm(common_peaks_reference_x_intensity ~ common_peaks_sample_x_intensity)
                ...
                regression_coefficients <-
                  lm.influence(linear_regression)$wt.res
                most_influencing_signals <-
                  paste(names(sort(regression_coefficients)), collapse = " , ")
                # Append this row to the global matrix
                slope_matrix[1, db] <- slope_sample
                most_influencing_signals_reference_samples_matrix[1, db] <-
                  most_influencing_signals
              } else {
                intensity_correlation_sample <- 0
              }
              ...
              number_of_signals_reference_samples_matrix[1, db] <-
                ncol(intensity_matrix_all)
              # Weighted correlation between samples (library + test samples) (samples must be as columns and features as test) - With weights
              if (intensity_correction_coefficient != 0 &&
                  intensity_correction_coefficient != 1 &&
                  correlation_method == "pearson") {
                ...
                intensity_correlation_sample <-
                  wtd.cors(x = intensity_matrix_all[2, ],
                           y = intensity_matrix_all[1, ],
                           weight = weights_vector)
              } else if (intensity_correction_coefficient == 1 ||
                         (
                           intensity_correction_coefficient != 0 &&
                ...
                if (ncol(intensity_matrix_all) >= 3) {
                  correlation_sample <-
                    cor.test(x = intensity_matrix_all[2, ],
                             y = intensity_matrix_all[1, ],
                             method = correlation_method)
                  intensity_correlation_sample <-
                    correlation_sample$estimate
              ...
              ## COUNTER 4 - REGRESSION CURVE
              linear_regression <-
                lm(intensity_matrix_all[1, ] ~ intensity_matrix_all[2, ])
              regression_slope <- linear_regression$coefficients[2]
          ...
          return(
            list(
              number_of_signals_samples = number_of_signals_samples,
              number_of_signals_reference_matrix = number_of_signals_reference_matrix,
              number_of_signals_reference_samples_matrix = number_of_signals_reference_samples_matrix,
              matching_signals_matrix = matching_signals_matrix,
              common_signals_reference_samples_matrix = common_signals_reference_samples_matrix,
              most_influencing_signals_reference_samples_matrix = most_influencing_signals_reference_samples_matrix,
              fit_matrix = fit_matrix,
              retrofit_matrix = retrofit_matrix,
              intensity_correlation_matrix = intensity_correlation_matrix,
      ...
      if ((is.logical(allow_parallelization) &&
        ...
        if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
          ...
          output_list <-
            mclapply(
              ...
              FUN = function(reference_sample_list)
                comparison_sample_db_subfunction_correlation(reference_sample_list, modality = correlation_mode),
              mc.cores = cpu_thread_number
            )
        } else if (Sys.info()[1] == "Windows") {
          #cpu_thread_number <- cpu_thread_number - 1
          ...
          clusterExport(
            ...
            varlist = c(
              ...
              "low_intensity_threshold_method",
        ...
        output_list <-
          foreach(
            i = 1:length(reference_sample_list),
            .packages = c("weights", "MALDIquant"),
            .export = c(
              "peak_picking",
              "signals_to_take",
              "align_and_filter_peaks",
            ...
            output_list[[i]] <-
              comparison_sample_db_subfunction_correlation(reference_sample_list[[i]], modality = correlation_mode)
          }
      ...
      for (ns in 1:number_of_samples) {
        ...
        if (is.null(matching_signals_matrix_all)) {
          matching_signals_matrix_all <-
            output_list[[ns]]$matching_signals_matrix
        } else {
          matching_signals_matrix_all <-
            rbind(matching_signals_matrix_all,
                  output_list[[ns]]$matching_signals_matrix)
        }
        # Common signals
        if (is.null(common_signals_reference_samples_matrix_all)) {
          common_signals_reference_samples_matrix_all <-
            output_list[[ns]]$common_signals_reference_samples_matrix
        } else {
          common_signals_reference_samples_matrix_all <-
            rbind(
              common_signals_reference_samples_matrix_all,
        ...
        if (is.null(most_influencing_signals_reference_samples_matrix_all)) {
          most_influencing_signals_reference_samples_matrix_all <-
            output_list[[ns]]$most_influencing_signals_reference_samples_matrix
        } else {
          most_influencing_signals_reference_samples_matrix_all <-
            rbind(
              most_influencing_signals_reference_samples_matrix_all,
        ...
        if (is.null(number_of_signals_reference_matrix_all)) {
          number_of_signals_reference_matrix_all <-
            output_list[[ns]]$number_of_signals_reference_matrix
        } else {
          number_of_signals_reference_matrix_all <-
            rbind(
              number_of_signals_reference_matrix_all,
        ...
        if (is.null(number_of_signals_reference_samples_matrix_all)) {
          number_of_signals_reference_samples_matrix_all <-
            output_list[[ns]]$number_of_signals_reference_samples_matrix
        } else {
          number_of_signals_reference_samples_matrix_all <-
            rbind(
              number_of_signals_reference_samples_matrix_all,
        ...
        if (is.null(fit_matrix_all)) {
          ...
          fit_matrix_all <-
            rbind(fit_matrix_all, output_list[[ns]]$fit_matrix)
        }
        # Retrofit
        if (is.null(retrofit_matrix_all)) {
          ...
          retrofit_matrix_all <-
            rbind(retrofit_matrix_all, output_list[[ns]]$retrofit_matrix)
        }
        # Intensity correlation
        if (is.null(intensity_correlation_matrix_all)) {
          intensity_correlation_matrix_all <-
            output_list[[ns]]$intensity_correlation_matrix
        } else {
          intensity_correlation_matrix_all <-
            rbind(
              intensity_correlation_matrix_all,
        ...
        if (is.null(pvalue_matrix_all)) {
          ...
          pvalue_matrix_all <-
            rbind(pvalue_matrix_all, output_list[[ns]]$pvalue_matrix)
        }
        # Slope
        if (is.null(slope_matrix_all)) {
          ...
          slope_matrix_all <-
            rbind(slope_matrix_all, output_list[[ns]]$slope_matrix)
        }
      }
      ##### Score calculation
      if (intensity_correction_coefficient != 0) {
        score <-
          log10(fit_matrix_all * retrofit_matrix_all * intensity_correlation_matrix_all *
                  1000)
      } else {
        score <-
          log10(fit_matrix_all * retrofit_matrix_all * intensity_correlation_matrix_all *
          ...
          if (score[r, w] >= score_threshold_values[2]) {
            score_matrix_output[r, w] <-
              paste0(
                ...
                "), ",
                "(F: ",
                matching_signals_matrix_all[r, w],
                " / ",
                output_list[[r]]$number_of_signals_samples,
                " = ",
                round(fit_matrix_all[r, w], digits = 3),
                " , ",
                "RF: ",
                matching_signals_matrix_all[r, w],
                " / ",
                number_of_signals_reference_matrix_all[r, w],
                " = ",
                round(retrofit_matrix_all[r, w], digits = 3),
                " , ",
                "Corr: ",
                round(intensity_correlation_matrix_all[r, w], digits = 3),
                " , ",
                "p: ",
                pvalue_matrix_all[r, w],
                " , ",
                "sl: ",
                slope_matrix_all[r, w],
                " , ",
                "ns: ",
                ifelse(
                  correlation_mode == "common signals",
                  matching_signals_matrix_all[r, w],
                  number_of_signals_reference_samples_matrix_all[r, w]
                ),
                ")"
              )
          } else if (score[r, w] < score_threshold_values[1]) {
      ...
      return(
        list(
          score_matrix_output = score_matrix_output,
          score_only_matrix_output = score_only_matrix_output,
          common_signals_reference_samples_matrix = common_signals_reference_samples_matrix_all,
          most_influencing_signals_reference_samples_matrix = most_influencing_signals_reference_samples_matrix_all
        )
      )
    }
  ...
  spectral_typer_score_signal_intensity <<-
    function(spectra_reference,
             ...
             signal_intensity_evaluation = c(
               "fixed percentage",
               "peak-wise adjusted percentage",
             ...
             low_intensity_threshold_method = "element-wise",
             tof_mode = "linear",
             intensity_tolerance_percent_threshold = 50,
             spectra_format = "fid",
             allow_parallelization = FALSE,
             score_threshold_values = c(1.7, 2),
             tolerance_ppm = NULL,
      ...
      if (is.null(reference_spectral_variability_list) ||
          length(reference_spectral_variability_list) == 0 ||
          is.null(test_spectral_variability_list) ||
          length(test_spectral_variability_list) == 0) {
        signal_intensity_evaluation <- "fixed percentage"
      } else if (!is.null(reference_spectral_variability_list) &&
                 length(reference_spectral_variability_list) > 0 &&
                 !is.null(test_spectral_variability_list) &&
                 length(test_spectral_variability_list) > 0) {
      ...
      for (spl in 1:number_of_samples) {
        ...
        if (signal_intensity_evaluation != "fixed percentage") {
          reference_sample_list[[spl]][["reference_spectral_variability_list"]] <-
            reference_spectral_variability_list
          reference_sample_list[[spl]][["test_spectral_variability_list"]] <-
            test_spectral_variability_list
          # Add the mean CV in reference_sample_list
          reference_sample_list[[spl]][["mean_cv_list_database"]] <-
            reference_spectral_variability_list$mean_cv_list
          reference_sample_list[[spl]][["mean_cv_sample"]] <-
            test_spectral_variability_list$mean_cv_list[[spl]]
        }
        reference_sample_list[[spl]][["spectra_reference"]] <-
          spectra_reference
        reference_sample_list[[spl]][["spectrum_sample"]] <-
      ...
      comparison_sample_db_subfunction_intensity <- function(x) {
        ...
        for (db in 1:reference_size) {
          ...
          if ((
            !is.null(reference_spectral_variability_list) &&
            length(reference_spectral_variability_list) > 0
          ) &&
          (
            !is.null(test_spectral_variability_list) &&
            ...
            if (length(peaks_reference_x@snr) == length(reference_spectral_variability_list$cv_list[[db]]) &&
                length(peaks_sample_x@snr) == length(test_spectral_variability_list$cv_list[[x$sample_ID]])) {
              peaks_reference_x@snr <-
                reference_spectral_variability_list$cv_list[[db]]
              peaks_sample_x@snr <-
                test_spectral_variability_list$cv_list[[x$sample_ID]]
            } else {
          ...
          if (signal_intensity_evaluation == "fixed percentage") {
            ...
            if (length(peaks_sample_x@mass) > 0 &&
              ...
              common_peaks_reference_sample <-
                intersect(peaks_sample_x@mass, peaks_reference_x@mass)
              # Extract the IDs (to match with their intensities)
              common_peaks_reference_id <-
                which(peaks_reference_x@mass %in% common_peaks_reference_sample)
              common_peaks_sample_id <-
                which(peaks_sample_x@mass %in% common_peaks_reference_sample)
              # Extract the intensities of the common peaks
              common_peak_intensities_database <-
                as.numeric(peaks_reference_x@intensity[common_peaks_reference_id])
              common_peak_intensities_sample <-
                as.numeric(peaks_sample_x@intensity[common_peaks_sample_id])
              # Find the matching intesities
              if (length(common_peaks_reference_sample) > 0) {
                intensity_matching_sample <-
                  length(which((
                    abs(
                      common_peak_intensities_database - common_peak_intensities_sample
                    ) * 100 / common_peak_intensities_database
                  ) <= intensity_tolerance_percent_threshold
                  ))
                # Fix the value to a relative value
                intensity_matching_sample <-
                  intensity_matching_sample / length(common_peaks_reference_sample)
              } else {
            ...
            # Append this row to the global matrix
            intensity_matching_matrix[1, db] <-
              intensity_matching_sample
          } else if (signal_intensity_evaluation == "peak-wise adjusted percentage" &&
                     (!is.null(x[["reference_spectral_variability_list"]]) &&
                      !is.null(x[["test_spectral_variability_list"]])) &&
                     (length(x[["reference_spectral_variability_list"]]) > 0 &&
                      length(x[["test_spectral_variability_list"]]) > 0)) {
            ...
            if (length(peaks_sample_x@mass) > 0 &&
              ...
              common_peak_intensities_sample <-
                as.numeric(peaks_sample_x@intensity[common_peaks_sample_id])
              common_peak_cv_database <-
                as.numeric(peaks_reference_x@snr[common_peaks_reference_id])
              common_peak_cv_sample <-
                as.numeric(peaks_sample_x@snr[common_peaks_sample_id])
              # Find the matching intensities (intervals accounting for the CV)
              if (length(common_peaks_reference_sample) > 0) {
                intensity_matching_sample <- 0
                for (z in 1:length(common_peaks_reference_sample)) {
                  if (!is.na(common_peak_cv_sample[z]) &&
                      !is.na(common_peak_cv_database[z])) {
                    sample_intensity_interval <-
                      c(
                        common_peak_intensities_sample[z] - common_peak_intensities_sample[z] * common_peak_cv_sample[z] /
                          100,
                        common_peak_intensities_sample[z] + common_peak_intensities_sample[z] * common_peak_cv_sample[z] /
                          100
                      )
                    database_intensity_interval <-
                      c(
                        common_peak_intensities_database[z] - common_peak_intensities_database[z] * common_peak_cv_database[z] /
                          100,
                        common_peak_intensities_database[z] + common_peak_intensities_database[z] * common_peak_cv_database[z] /
                          100
                      )
                    if ((
                      database_intensity_interval[1] <= sample_intensity_interval[1] &&
                      sample_intensity_interval[1] <= database_intensity_interval[2]
                    ) ||
                    (
                      sample_intensity_interval[1] <= database_intensity_interval[1] &&
                      database_intensity_interval[1] <= sample_intensity_interval[2]
                    )) {
                    ...
                    for (z in 1:length(common_peaks_reference_sample)) {
                      sample_intensity_interval <-
                        c(
                          common_peak_intensities_sample[z] - common_peak_intensities_sample[z] * average_cv_sample /
                            100,
                          common_peak_intensities_sample[z] + common_peak_intensities_sample[z] * average_cv_sample /
                            100
                        )
                      database_intensity_interval <-
                        c(
                          common_peak_intensities_database[z] - common_peak_intensities_database[z] * average_cv_database /
                            100,
                          common_peak_intensities_database[z] + common_peak_intensities_database[z] * average_cv_database /
                            100
                        )
        ...
        return(
          list(
            ...
            fit_matrix = fit_matrix,
            retrofit_matrix = retrofit_matrix,
            intensity_matching_matrix = intensity_matching_matrix
          )
        )
      }
      ##### Run the function for each element of the reference_sample_list (= each sample) (each sample gets compared with the database)
      if ((is.logical(allow_parallelization) &&
        ...
        if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
          ...
          output_list <-
            mclapply(
              reference_sample_list,
              FUN = function(reference_sample_list)
                comparison_sample_db_subfunction_intensity(reference_sample_list),
              mc.cores = cpu_thread_number
            )
        } else if (Sys.info()[1] == "Windows") {
          #cpu_thread_number <- cpu_thread_number - 1
          ...
          clusterExport(
            cl = cls,
            varlist = c(
              "comparison_sample_db_subfunction_intensity",
              "reference_size",
              "tof_mode",
              "peaks_filtering_percentage_threshold",
              "low_intensity_percentage_threshold",
        ...
        output_list <-
          foreach(
            ...
            output_list[[i]] <-
              comparison_sample_db_subfunction_intensity(reference_sample_list[[i]])
          }
        names(output_list) <- list_names
      } else {
        output_list <-
      ...
      for (ns in 1:number_of_samples) {
        ...
        if (is.null(intensity_matching_matrix_all)) {
          intensity_matching_matrix_all <-
            output_list[[ns]]$intensity_matching_matrix
        } else {
          intensity_matching_matrix_all <-
            rbind(intensity_matching_matrix_all,
                  output_list[[ns]]$intensity_matching_matrix)
        }
      }
      ##### Score calculation
      score <-
        log10(fit_matrix_all * retrofit_matrix_all * intensity_matching_matrix_all *
                1000)
      ##### Output the classification
      # Score components
      score_matrix_output <-
          ...
          if (score[r, w] >= score_threshold_values[2]) {
            score_matrix_output[r, w] <-
              paste0(
                ...
                " , ",
                "IntMtch: ",
                round(intensity_matching_matrix_all[r, w], digits = 3),
                " , ",
                "ns: ",
                matching_signals_matrix_all[r, w],
                ")"
  ...
  # Each sample gets compared with each entry in the database, separately.
  # Parallel implemented.
  spectral_typer_score_similarity_index <<-
    function(spectra_reference,
             spectra_test,
             filepath_reference,
             filepath_test,
             class_list_library = NULL,
             peaks_filtering_percentage_threshold = 5,
             low_intensity_percentage_threshold = 0.1,
             low_intensity_threshold_method = "element-wise",
             tof_mode = "linear",
             spectra_format = "fid",
             allow_parallelization = FALSE,
      ...
      ##### Define the function for lapply
      # x = each element of the reference_sample_list
      comparison_sample_db_subfunction_similarity_index <-
        function(x) {
          ## Retrieve the values from x
          reference_vector <- x$reference_vector
          reference_size <- length(reference_vector)
          ...
          for (db in 1:reference_size) {
            ...
            ## COUNTER 3
            # Symmetry -> comparison between intensities
            similarity_index_matrix_global <-
              intensityMatrix(peaks_all, spectra_all)
            rownames(similarity_index_matrix_global) <-
              c("Reference", "Sample")
            ...
            for (z in 1:ncol(similarity_index_matrix_global)) {
              A <-
                A + (similarity_index_matrix_global[1, z] * similarity_index_matrix_global[2, z])
            }
            B <- 0
            for (z in 1:ncol(similarity_index_matrix_global)) {
          ...
          return(
            list(
              ...
              fit_matrix = fit_matrix,
              retrofit_matrix = retrofit_matrix,
              similarity_index_matrix = similarity_index_matrix
            )
          )
        }
      ##### Run the function for each element of the reference_sample_list (= each sample) (each sample gets compared with the database)
      if ((is.logical(allow_parallelization) &&
        ...
        if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
          ...
          output_list <-
            mclapply(
              reference_sample_list,
              FUN = function(reference_sample_list)
                comparison_sample_db_subfunction_similarity_index(reference_sample_list),
              mc.cores = cpu_thread_number
            )
        } else if (Sys.info()[1] == "Windows") {
          #cpu_thread_number <- cpu_thread_number - 1
        ...
        output_list <-
          foreach(
            ...
            output_list[[i]] <-
              comparison_sample_db_subfunction_similarity_index(reference_sample_list[[i]])
          }
        names(output_list) <- list_names
      } else {
        output_list <-
      ...
      for (ns in 1:number_of_samples) {
        ...
        if (is.null(similarity_index_matrix_all)) {
          similarity_index_matrix_all <-
            output_list[[ns]]$similarity_index_matrix
        } else {
          similarity_index_matrix_all <-
            rbind(similarity_index_matrix_all,
                  output_list[[ns]]$similarity_index_matrix)
        }
      }
      ##### Score calculation
      score <-
        log10(fit_matrix_all * retrofit_matrix_all * similarity_index_matrix_all *
                1000)
      ##### Output the classification
      # Score components
      score_matrix_output <-
          ...
          if (score[r, w] >= score_threshold_values[2]) {
            score_matrix_output[r, w] <-
              paste0(
                ...
                " , ",
                "SI: ",
                round(similarity_index_matrix_all[r, w], digits = 3),
                ")"
              )
          } else if (score[r, w] < score_threshold_values[1]) {
            score_matrix_output[r, w] <-
  ...
  spectral_variability_estimation <<-
    function(spectra,
             folder_list = NULL,
             peak_picking_SNR = 3,
             peak_picking_algorithm = "SuperSmoother",
      ...
      for (f in 1:length(folder_list)) {
        ...
        if (length(spectra_temp) > 0 && length(peaks_temp) > 0) {
          intensity_matrix_list[[folder_list[f]]] <-
            intensityMatrix(peaks_temp, spectra_temp)
        }
      }
      ...
      for (i in 1:length(intensity_matrix_list)) {
        ...
        for (cl in 1:ncol(intensity_matrix_temp)) {
          ...
          mean_column <-
            mean(intensity_matrix_temp[, cl], na.rm = TRUE)
        ...
        mean_cv_list[[names(intensity_matrix_list)[i]]] <-
          mean(cv_vector, na.rm = TRUE)
      }
      ...
      estimated_intensity_tolerance_percent <-
        round(mean(unlist(mean_cv_list), na.rm = TRUE))
      ### Return
      return(
        list(
          mean_cv_list = mean_cv_list,
          cv_list = cv_list,
          estimated_intensity_tolerance_percent = estimated_intensity_tolerance_percent,
          cv_matrix = cv_matrix
        )
      )
    }
  ...
  generate_adjacency_matrix <<-
    function(peaklist_matrix,
      ...
      correlation_matrix <-
        cor(peaklist_matrix_t, method = correlation_method)
      ...
      if (pvalue_threshold == 0 || is.null(pvalue_threshold)) {
        ...
        adjacency_matrix <-
          apply(
            ...
            MARGIN = c(1, 2),
            FUN = function(x)
              matrix_replacement_subfunction(x, threshold = correlation_threshold)
          )
        ...
        global_matrix <-
          matrix(
            "",
            nrow = nrow(correlation_matrix_pvalue),
            ncol = ncol(correlation_matrix_pvalue)
          )
        ...
        for (rw in 1:nrow(correlation_matrix_pvalue)) {
          for (cl in 1:ncol(correlation_matrix_pvalue)) {
            global_matrix[rw, cl] <-
              paste(correlation_matrix[rw, cl],
                    correlation_matrix_pvalue[rw, cl],
                    sep = ",")
          }
        }
        ##### Generate the function to apply to the matrix (x = matrix entry)
        matrix_replacement_subfunction <-
          function(x, coeff_threshold, p_threshold) {
            # Split the entry
            splitted_x <- as.numeric(unlist(strsplit(x, ",")))
            if (abs(splitted_x[1]) >= coeff_threshold &&
                abs(splitted_x[2]) <= p_threshold) {
              x <- 1
            } else {
              x <- 0
            }
        ...
        adjacency_matrix <-
          apply(
            ...
            MARGIN = c(1, 2),
            FUN = function(x)
              matrix_replacement_subfunction(x, coeff_threshold = correlation_threshold, p_threshold = pvalue_threshold)
          )
        ##### Fix the column and row names
        rownames(adjacency_matrix) <- rownames(peaklist_matrix)
        colnames(adjacency_matrix) <- rownames(peaklist_matrix)
    ...
    function(spectra,
      ...
      if (!is.null(custom_feature_vector)) {
        ...
        custom_feature_vector <-
          sort(as.numeric(unique(custom_feature_vector)))
        # Retrieve the maximum and minimum data points in the spectra
        spectra_peaks <- numeric()
        if (isMassSpectrumList(spectra)) {
          for (sp in 1:length(spectra)) {
            spectra_peaks <-
              append(spectra_peaks, c(spectra[[sp]]@mass[1], spectra[[sp]]@mass[length(spectra[[sp]]@mass)]))
          }
        } else {
          spectra_peaks <-
            append(spectra_peaks, c(spectra@mass[1], spectra@mass[length(spectra@mass)]))
        }
        spectra_peaks <- sort(as.numeric(unique(spectra_peaks)))
        ### Check the compatibility between the spectra and the provided mass list
        if (spectra_peaks[1] <= custom_feature_vector[1] &&
            spectra_peaks[length(spectra_peaks)] >= custom_feature_vector[length(custom_feature_vector)]) {
        ...
        if (feature_compatibility == TRUE) {
          ...
          peak_alignment_subfunction <-
            function(peaks,
              ...
              for (l in 1:length(reference_masses)) {
                ...
                peak_id <-
                  which(abs((
                    as.numeric(peaks@mass) - as.numeric(reference_masses[l])
                  ) / as.numeric(reference_masses[l]) * 10 ^ 6
                  ) <= tolerance_ppm)
                ...
                if (length(peak_id == 1)) {
                  ...
                  peaks@mass[peak_id[1]] <-
                    as.numeric(reference_masses[l])
                }
              }
              # Return
              return(peaks)
                ...
                peaks <-
                  mclapply(
                    ...
                    FUN = function(peaks)
                      peak_alignment_subfunction(
                        peaks = peaks,
                        reference_masses = custom_feature_vector,
                        tolerance_ppm = tolerance_ppm
                      ),
                    mc.cores = cpu_thread_number
                  )
          ...
          if (isMassSpectrumList(spectra)) {
            fake_spectrum <-
              createMassSpectrum(
                ...
                metaData = list(name = "Fake spectrum")
              )
          } else if (isMassSpectrum(spectra)) {
            fake_spectrum <-
              createMassSpectrum(
          ...
          fake_peaks <-
            createMassPeaks(
              mass = as.numeric(custom_feature_vector),
              intensity = rep(1, length(custom_feature_vector)),
              snr = rep(peak_picking_SNR, length(custom_feature_vector)),
              metaData = list(name = "Fake peaklist")
            )
          ...
          # Generate the intensity matrix
          intensity_matrix_all <-
            intensityMatrix(peaks_all, spectra_all)
          # Remove the first row (corresponding to the fake spectrum)
          intensity_matrix_all <-
            intensity_matrix_all[2:nrow(intensity_matrix_all),]
          # Keep only the columns of interest
          if (is.matrix(intensity_matrix_all)) {
            final_peaklist_matrix <-
              intensity_matrix_all[, as.character(custom_feature_vector)]
          } else if (is.vector(intensity_matrix_all)) {
            final_peaklist_matrix <-
              as.matrix(rbind(intensity_matrix_all[names(intensity_matrix_all) %in% as.character(custom_feature_vector)]))
          }
          # Rownames
          if (is.null(names(spectra))) {
            rownames(final_peaklist_matrix) <-
              seq(1, nrow(final_peaklist_matrix), by = 1)
          } else {
  ...
  rearrange_spectral_dataset <<-
    function(spectra,
             rearranging_method = c("space", "random"),
             seed = NULL) {
      ##### Install and load the required packages
      require(XML)
      require(MALDIquant)
      ...
      if (isMassSpectrumList(spectra)) {
        ...
        if (rearranging_method == "space") {
          ...
          if (!is.null(spectra[[1]]@metaData$imaging$pos)) {
            ...
            rectangle_coordinates[1, 1] <-
              min(spectral_coordinates[, 1])
            rectangle_coordinates[2, 1] <-
              max(spectral_coordinates[, 1])
            rectangle_coordinates[1, 2] <-
              min(spectral_coordinates[, 2])
            rectangle_coordinates[2, 2] <-
              max(spectral_coordinates[, 2])
            ...
            if (widest_coordinate == "x") {
              ...
              spectral_coordinates_sorted <-
                spectral_coordinates[order(spectral_coordinates[, 2],
                                           spectral_coordinates[, 1],
                                           decreasing = FALSE),]
              # Extract the row names, which are the ID numbers of the spectral list
              spectra_ID <-
                as.integer(rownames(spectral_coordinates_sorted))
              ...
              spectral_coordinates_sorted <-
                spectral_coordinates[order(spectral_coordinates[, 1],
                                           spectral_coordinates[, 2],
                                           decreasing = FALSE),]
              # Extract the row names, which are the ID numbers of the spectral list
              rearranged_spectra_IDs <-
                as.integer(rownames(spectral_coordinates_sorted))
              # Rearrange the spectral list
          ...
          rearranged_spectra_IDs <-
            sample(1:length(spectra), size = length(spectra))
          # Rearrange the spectral list
          spectra_riarranged <- spectra[rearranged_spectra_IDs]
  ...
  partition_spectral_dataset <<-
    function(spectra,
             partitioning_method = c("space", "random", "hca"),
        ...
        if (partitioning_method == "space") {
          ...
          if (!is.null(spectra[[1]]@metaData$imaging$pos)) {
            ...
            if (widest_coordinate == "x") {
              ...
              interval_width <-
                ceiling(
                  abs(rectangle_coordinates[2, 1] - rectangle_coordinates[1, 1]) / number_of_partitions
                )
              # Define the separating points
              interval_points <-
                seq.int(from = rectangle_coordinates[1, 1],
                        to = rectangle_coordinates[2, 1],
                        by = interval_width)
              ...
              for (p in 1:(length(interval_points) - 1)) {
                ...
                for (s in 1:length(spectra)) {
                  # Populate the list...
                  if (spectra[[s]]@metaData$imaging$pos[1] >= interval_points[p] &&
                      spectra[[s]]@metaData$imaging$pos[1] < interval_points[p + 1]) {
              ...
              # Define the intervals
              interval_width <-
                ceiling((rectangle_coordinates[2, 2] - rectangle_coordinates[1, 2]) / number_of_partitions
                )
              # Define the separating points
              interval_points <-
                seq.int(rectangle_coordinates[1, 2],
                        rectangle_coordinates[2, 2],
                        by = interval_width)
              # Add the maximum coordinate to the interval points (if not already present)
              ...
              for (p in 1:(length(interval_points) - 1)) {
                ...
                for (s in 1:length(spectra)) {
                  # Populate the list...
                  if (spectra[[s]]@metaData$imaging$pos[2] >= interval_points[p] &&
                      spectra[[s]]@metaData$imaging$pos[2] < interval_points[p + 1]) {
                    spectra_interval <- append(spectra_interval, spectra[[s]])
                  }
                }
                # Add the spectra with the max coordinate
          ...
          spectra_partitioned_IDs <-
            createFolds(
              y = rep("spectra", length(spectra)),
          ...
          for (s in 1:length(spectra_partitioned_IDs)) {
            ...
            spectra_partitioned[[s]] <-
              spectra[spectra_partitioned_IDs[[s]]]
          }
          ...
          peaks <-
            peak_picking(
              ...
              peak_picking_algorithm = "SuperSmoother",
              tof_mode = tof_mode,
              SNR = ifelse(tof_mode == "reflectron", 5, 3),
              allow_parallelization = FALSE
            )
          peaks <-
            align_and_filter_peaks(
  ...
  extract_feature_list_from_model_list <<-
    function(filepath_R, features_to_return = 20) {
      ...
      for (md in 1:length(list_of_models)) {
        if (length(model_list[[md]]$features_model) > highest_number_of_features) {
          highest_number_of_features <-
            length(model_list[[md]]$features_model)
        }
      }
      ### Initialize the feature matrix
      model_features_matrix <-
        matrix(0, nrow = highest_number_of_features, ncol = length(list_of_models))
      ...
      for (md in 1:length(list_of_models)) {
        ...
        # Fill the dataframe
        feature_df <-
          data.frame(features = cbind(feature_vector),
                     rank = cbind(seq(1, length(
                       feature_vector
                     ), by = 1)))
        if (is.null(feature_dataframe)) {
          ...
          feature_dataframe <-
            rbind(feature_dataframe, feature_df[feature_df$feature_vector != "",])
        }
      }
      # Sort the feature dataframe according to the rank
      feature_dataframe <-
        feature_dataframe[order(feature_dataframe$rank),]
      ...
      if (features_to_return > 0 &&
          features_to_return < length(model_features)) {
      ...
      return(
        list(
          model_features = model_features,
          model_features_matrix = model_features_matrix
        )
      )
    }
  ...
  extract_common_features_from_model_list <<-
    function(filepath_R, features_to_return = 20) {
      ...
      common_features_vector <-
        unique(feature_vector[duplicated(feature_vector) == TRUE])
      ### Remove the X from the features
      ...
      ### Output a certain number of features
      if (features_to_return > 0 &&
          features_to_return < length(common_features_vector)) {
  ...
  extract_performance_matrix_from_model_list <<-
    function(filepath_R) {
      if (!is.list(filepath_R)) {
        ### LOAD THE R WORKSPACE WITH THE MODEL LIST
        # Create a temporary environment
      ...
      for (md in 1:length(list_of_models)) {
        ...
        if (is.null(model_performance_matrix)) {
          ...
          colnames(single_model_performance) <-
            c("Model", names(model_performance))
          ...
          single_model_performance[1, 2] <- model_performance
          model_performance_matrix <-
            rbind(model_performance_matrix, single_model_performance)
        }
      }
      ### Return
  ...
  genetic_algorithm_graph <<-
    function(input_adjacency_matrix,
             graph_type = "Preferential",
             vertex_mutation = TRUE,
             triangle_mutation = TRUE,
             allow_parallelization = FALSE,
             number_of_high_degree_vertices_for_subgraph = 0,
             vertices_not_in_induced_subgraph = c("independent", "reassigned"),
             vertex_independency_threshold = 200,
             iterations_with_no_change = 5,
             max_GA_generations = 10,
             seed = 12345) {
      ##### Install and load the required packages
      ...
      ### Generate the graph
      initial_graph <-
        graph_from_adjacency_matrix(input_adjacency_matrix)
      ### Generate a subgraph with only the high-degree vertices
      if (!is.null(number_of_high_degree_vertices_for_subgraph) &&
          number_of_high_degree_vertices_for_subgraph > 0 &&
          number_of_high_degree_vertices_for_subgraph < length(V(initial_graph))) {
        # Generate the dataframe with the vertex IDs and their degree
        degree_dataframe <-
          data.frame(ID = seq(1:length(V(
            initial_graph
          ))),
          degree = degree(initial_graph))
        # Sort it according to the degree
        degree_dataframe_sorted <-
          degree_dataframe[order(degree_dataframe$degree, decreasing = TRUE),]
        input_graph <-
          induced_subgraph(initial_graph, vids = degree_dataframe_sorted$ID[1:number_of_high_degree_vertices_for_subgraph], impl = "auto")
        # Regenerate the adjacency matrix according to the new graph
        input_adjacency_matrix <-
          as.matrix(as_adjacency_matrix(input_graph))
      } else {
      ...
      independent_vertex_number_sampling <-
        round(0.28 * vertex_number)
      ...
      quasi_independent_vertices <-
        which(degree_matrix[, "degree_sequence"] <= degree_threshold_independency)
      # Compute the number of quasi-independent vertices
      independent_vertex_number_graph <-
        length(quasi_independent_vertices)
      ...
      triangle_matrix <-
        matrix(triangle_array,
               nrow = length(triangle_array) / 3,
               byrow = TRUE)
      ...
      FITN_SplitGraph <- function(chromosome) {
        ...
        chromosome_adjacency_matrix <-
          matrix(0, nrow = chromosome_size, ncol = chromosome_size)
        # Fill in the adjacency matrix
        chromosome_adjacency_matrix[which(chromosome == 1), ] <-
          rep(chromosome, each = number_of_ones)
        # Set the diagonal to zero
        diag(chromosome_adjacency_matrix) <- 0
        # Generate the graph from the adjacency matrix
        chromosome_graph <-
          graph_from_adjacency_matrix(chromosome_adjacency_matrix, mode = "undirect")
        ...
        fitness_GA <-
          -sum(abs(input_adjacency_matrix - chromosome_adjacency_matrix)) / 2
        return(fitness_GA)
      }
      ###################### SPLIT MUTATION
      ...
      SplitMutation <- function(object, parent_ind) {
        ...
        if (triangle_mutation == TRUE) {
          ...
          rsamp_RowIX <-
            runif(triangle_number_sampling, 1, triangle_number_graph)
        ...
        if (vertex_mutation == TRUE) {
          # Compute the uniform distribution
          rsamp_IndVert <-
            runif(independent_vertex_number_sampling,
                  1,
                  independent_vertex_number_graph)
          ...
          vertex_sample <-
            quasi_independent_vertices[RoundedSampl_IndVertIX]
      ...
      output_adjacency_matrix <-
        as.matrix(as_adjacency_matrix(final_graph, type = "both"))
      rownames(output_adjacency_matrix) <-
        rownames(input_adjacency_matrix)
      colnames(output_adjacency_matrix) <-
        colnames(input_adjacency_matrix)
      ...
      if (!is.null(number_of_high_degree_vertices_for_subgraph) &&
        ...
        if (is.matrix(GA_model@solution)) {
          ...
          names(final_chromosome) <-
            degree_dataframe_sorted$ID[1:number_of_high_degree_vertices_for_subgraph]
        } else {
        ...
        if (vertices_not_in_induced_subgraph == "independent") {
          ...
          vertex_IDs_outside_chromosome <-
            as.numeric(V(initial_graph)[!(V(initial_graph) %in% vertex_IDs_chromosome)])
          # Generate the vector of 0 (all the vertices go into the independent set)
          vertices_outside_chromosome <-
            numeric(length = length(vertex_IDs_outside_chromosome))
          names(vertices_outside_chromosome) <-
            vertex_IDs_outside_chromosome
          # Add this vector to the final chromosome
          final_chromosome <-
            append(final_chromosome, vertices_outside_chromosome)
          # Convert it into a dataframe to sort it according to the names
          final_chromosome_df <-
            as.data.frame(cbind(
              ID = as.numeric(names(final_chromosome)),
              Value = as.numeric(final_chromosome)
            ))
          final_chromosome_df <-
            final_chromosome_df[order(final_chromosome_df$ID),]
      ...
      return(
        list(
          GA_model = GA_model,
          GA_model_solution = best_solution,
          input_graph = input_graph,
          final_graph = final_graph,
          output_adjacency_matrix = output_adjacency_matrix,
    ...
    function(final_chromosome_GA,
      ...
      if (isMassSpectrumList(spectra_clique_for_plotting)) {
        for (spp in 1:length(spectra_clique_for_plotting)) {
          spectra_clique_for_plotting[[spp]]@intensity <-
            rep(1, length(spectra_clique_for_plotting[[spp]]@intensity))
        }
      } else if (isMassSpectrum(spectra_clique_for_plotting)) {
        spectra_clique_for_plotting@intensity <-
          rep(1, length(spectra_clique_for_plotting@intensity))
      ...
      if (isMassSpectrumList(spectra_independent_for_plotting)) {
        for (spp in 1:length(spectra_independent_for_plotting)) {
          spectra_independent_for_plotting[[spp]]@intensity <-
            rep(0.5,
                length(spectra_independent_for_plotting[[spp]]@intensity))
        }
      } else if (isMassSpectrum(spectra_independent_for_plotting)) {
        spectra_independent_for_plotting@intensity <-
          rep(0.5,
              length(spectra_independent_for_plotting@intensity))
      ...
      spectra_all_for_plotting <-
        append(spectra_clique_for_plotting,
               spectra_independent_for_plotting)
      ...
      return(
        list(
          spectra_clique = spectra_clique,
          spectra_independent = spectra_independent,
          spectra_for_plotting = spectra_all_for_plotting
        )
      )
    }
  ...
  graph_MSI_segmentation <<-
    function(filepath_imzml,
             preprocessing_parameters = list(
               ...
               normalization_algorithm = "TIC",
               normalization_mass_range = NULL,
               process_spectra_in_packages_of = 0,
               spectral_alignment_algorithm = NULL
             ),
             tolerance_ppm = NULL,
             allow_parallelization = FALSE,
             ...
             custom_feature_vector = NULL,
             correlation_method_for_adjacency_matrix = "pearson",
             correlation_threshold_for_adjacency_matrix = 0.90,
             pvalue_threshold_for_adjacency_matrix = 0.05,
             number_of_high_degree_vertices_for_subgraph = 0,
             vertices_not_in_induced_subgraph = c("independent", "reassigned"),
             max_GA_generations = 10,
      ...
      if (!isMassSpectrumList(filepath_imzml) &&
          !isMassSpectrum(filepath_imzml)) {
        if (!is.null(preprocessing_parameters$mass_range)) {
          spectra <-
            importImzMl(filepath_imzml, massRange = preprocessing_parameters$mass_range)
        } else {
          spectra <- importImzMl(filepath_imzml)
        }
      } else if (isMassSpectrumList(filepath_imzml) ||
                 isMassSpectrum(filepath_imzml)) {
        spectra <- filepath_imzml
      }
      ### Identify the original spectra by setting the names (better unique identification of the spectra)
      spectra <-
        replace_sample_name_list(spectra, spectra_format = spectra_format)
      ### Rearrange spectra for reproducibility
      ...
      if (!is.null(peaklist_matrix)) {
        ...
        input_adjacency_matrix <-
          generate_adjacency_matrix(
            peaklist_matrix,
            correlation_method = correlation_method_for_adjacency_matrix,
            correlation_threshold = correlation_threshold_for_adjacency_matrix,
            pvalue_threshold = pvalue_threshold_for_adjacency_matrix
          )
        # Run the genetic algorithm
        GA_output <-
          genetic_algorithm_graph(
            ...
            max_GA_generations = max_GA_generations,
            seed = seed,
            number_of_high_degree_vertices_for_subgraph = number_of_high_degree_vertices_for_subgraph,
            vertices_not_in_induced_subgraph = vertices_not_in_induced_subgraph,
            allow_parallelization = allow_parallelization,
            iterations_with_no_change = iterations_with_no_change
          )
        # Record the plots
        ...
        MS_from_GA <-
          from_GA_to_MS(final_chromosome_GA = GA_output$final_chromosome,
                        spectra = spectra)
        ...
        slices <-
          msiSlices(
            spectra_all_for_plotting,
            center = spectra_all_for_plotting[[1]]@mass[(length(spectra_all_for_plotting[[1]]@mass) /
                                                           2)],
            tolerance = 1,
            adjust = TRUE,
            method = "median"
        ...
        if (plot_figures == TRUE) {
          ...
          if ("legend" %in% plot_legends) {
            ...
            legend(
              x = "bottomright",
              legend = c("clique", "independent set"),
              fill = c("red", "green"),
              xjust = 0.5,
              yjust = 0.5
            )
          ...
          if ("sample name" %in% plot_legends) {
            legend(
              x = "topright",
              legend = spectra_all_for_plotting[[1]]@metaData$file[1],
              xjust = 0.5,
              yjust = 0.5
            )
          }
        ...
        return(
          list(
            spectra_for_plotting = spectra_all_for_plotting,
            spectra_clique = MS_from_GA$spectra_clique,
            spectra_independent = MS_from_GA$spectra_independent,
            msi_segmentation = msi_segmentation,
            graph_spectra_plot = graph_spectra_plot,
            final_graph_plot = final_graph_plot,
