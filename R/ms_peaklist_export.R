ms_peaklist_export <- function() {
  ...
  github_R_url <-
    "https://raw.githubusercontent.com/gmanuel89/MS-Peaklist-Export/master/MS%20PEAKLIST%20EXPORT.R"
  ### GitHub URL of the program's WIKI
  github_wiki_url <-
    "https://github.com/gmanuel89/MS-Peaklist-Export/wiki"
  ...
  low_intensity_peak_removal_threshold_method_value <-
    "element-wise"
  allow_parallelization_value <- "NO"
  transform_data_value <- "None"
  smoothing_value <-
    paste0("YES", "\n( ", "Savitzky-Golay", ",\n" , "medium", " )")
  baseline_subtraction_value <-
    paste0("YES", "\n( ", "SNIP", ",\nIterations: ", "200", " )")
  ...
  preprocessing_window_function <- function() {
    ...
    transform_data_choice <- function() {
      ...
      } else if (transform_data_algorithm_input == "" ||
                 transform_data_algorithm_input == "None") {
        transform_data_algorithm <- NULL
      }
      ...
      transform_data_value_label <-
        tklabel(
          preproc_window,
          text = transform_data_value,
          font = label_font,
          bg = "white",
          width = 20,
          height = 2
    ...
    smoothing_choice <- function() {
      ...
      smoothing_algorithm_input <-
        select.list(
          c("Savitzky-Golay", "Moving Average", "None"),
          title = "Smoothing algorithm",
          multiple = FALSE,
          preselect = "SavitzkyGolay"
        )
      # Raise the focus on the preproc window
      tkraise(window)
      tkraise(preproc_window)
      # Default and fix
      if (smoothing_algorithm_input == "" ||
          smoothing_algorithm_input == "Savitzky-Golay") {
        smoothing_algorithm <- "SavitzkyGolay"
      ...
      if (!is.null(smoothing_algorithm)) {
        smoothing_strength <-
          select.list(
            c("small", "medium", "strong", "stronger"),
    ...
    baseline_subtraction_choice <- function() {
      ...
      baseline_subtraction_algorithm <-
        select.list(
          c("SNIP", "TopHat", "ConvexHull", "median", "None"),
          title = "Baseline subtraction algorithm",
          multiple = FALSE,
          preselect = "SNIP"
        )
      # Raise the focus on the preproc window
      ...
      if (!is.null(baseline_subtraction_algorithm) &&
        ...
        baseline_subtraction_algorithm_parameter <-
          tclvalue(baseline_subtraction_algorithm_parameter2)
        baseline_subtraction_algorithm_parameter_value <-
          as.character(baseline_subtraction_algorithm_parameter)
        baseline_subtraction_algorithm_parameter <-
          as.integer(baseline_subtraction_algorithm_parameter)
        ...
        baseline_subtraction_value <-
          paste0(
            ...
            " )"
                 ...
                 baseline_subtraction_algorithm == "ConvexHull") {
        baseline_subtraction_value <-
          paste0("YES", "\n( ", baseline_subtraction_algorithm, ")")
      } else {
        baseline_subtraction_value <- "None"
      }
      baseline_subtraction_value_label <-
        tklabel(
          preproc_window,
          text = baseline_subtraction_value,
          font = label_font,
          bg = "white",
          width = 20,
          height = 3
    ...
    normalization_choice <- function() {
      ...
      if (!is.null(normalization_algorithm) &&
        ...
        normalization_mass_range_value <-
          as.character(normalization_mass_range)
        if (normalization_mass_range != 0 &&
            normalization_mass_range != "") {
          normalization_mass_range <-
            unlist(strsplit(normalization_mass_range, ","))
          normalization_mass_range <-
            as.numeric(normalization_mass_range)
        } else if (normalization_mass_range == 0 ||
                   normalization_mass_range == "") {
          normalization_mass_range <- NULL
        }
      }
      ...
      if (!is.null(normalization_algorithm) &&
        ...
        normalization_value <-
          paste0("YES", "\n( ", normalization_algorithm, " )\n")
      } else if (!is.null(normalization_algorithm) &&
                 normalization_algorithm == "TIC") {
      ...
      normalization_value_label <-
        tklabel(
          preproc_window,
          text = normalization_value,
          font = label_font,
          bg = "white",
          width = 20,
          height = 4
    ...
    spectral_alignment_choice <- function() {
      ...
      spectral_alignment_algorithm <-
        select.list(
          c("cubic", "quadratic", "linear", "lowess", "None"),
          title = "Spectral alignment algorithm",
          multiple = FALSE,
          preselect = "cubic"
        )
      # Raise the focus on the preproc window
      ...
      if (!is.null(spectral_alignment_algorithm)) {
        spectral_alignment_reference <-
          select.list(
            c("auto", "average spectrum", "skyline spectrum"),
            title = "Spectral alignment reference",
            multiple = FALSE,
            preselect = "average spectrum"
          )
        # Raise the focus on the preproc window
        tkraise(window)
        tkraise(preproc_window)
      ...
      spectral_alignment_value_label <-
        tklabel(
          preproc_window,
          text = spectral_alignment_value,
          font = label_font,
          bg = "white",
          width = 20,
          height = 3
    ...
    tof_mode_choice <- function() {
      ...
      tof_mode <-
        select.list(c("Linear", "Reflectron"), title = "TOF mode")
      # Raise the focus on the preproc window
      tkraise(window)
      tkraise(preproc_window)
      # Default
    ...
    commit_preprocessing_function <- function() {
      ...
      mass_range_value <-
        as.character(paste(mass_range[1], ",", mass_range[2]))
      # Preprocessing
      preprocess_spectra_in_packages_of <-
        tclvalue(preprocess_spectra_in_packages_of2)
      preprocess_spectra_in_packages_of <-
        as.integer(preprocess_spectra_in_packages_of)
      preprocess_spectra_in_packages_of_value <-
        as.character(preprocess_spectra_in_packages_of)
      # Preprocessing
      ...
      preprocessing_parameters <<-
        list(
          mass_range = mass_range,
          transformation_algorithm = transform_data_algorithm,
          smoothing_algorithm = smoothing_algorithm,
          smoothing_strength = smoothing_strength,
    ...
    preprocess_spectra_in_packages_of_label <-
      tklabel(
        preproc_window,
    ...
    preprocess_spectra_in_packages_of_entry <-
      tkentry(
        preproc_window,
        textvariable = preprocess_spectra_in_packages_of2,
        font = entry_font,
        bg = "white",
        width = 10,
        justify = "center"
    ...
    tof_mode_entry <-
      tkbutton(
        preproc_window,
        text = "Choose the TOF mode",
        command = tof_mode_choice,
        font = button_font,
        bg = "white",
        width = 20
      )
    ...
    # Transform the data
    transform_data_button <-
      tkbutton(
        preproc_window,
        text = "Data transformation",
        command = transform_data_choice,
        font = button_font,
        bg = "white",
        width = 20
      )
    ...
    smoothing_button <-
      tkbutton(
        preproc_window,
        text = "Smoothing",
        command = smoothing_choice,
        font = button_font,
        bg = "white",
        width = 20
      )
    # Baseline subtraction
    baseline_subtraction_button <-
      tkbutton(
        preproc_window,
        text = "Baseline subtraction",
        command = baseline_subtraction_choice,
        font = button_font,
        bg = "white",
        width = 20
      )
    baseline_subtraction_algorithm_parameter_entry <-
      tkentry(
        preproc_window,
        textvariable = baseline_subtraction_algorithm_parameter2,
        font = entry_font,
        bg = "white",
        width = 10,
        justify = "center"
    ...
    # Normalization
    normalization_button <-
      tkbutton(
        preproc_window,
        text = "Normalization",
        command = normalization_choice,
        font = button_font,
        bg = "white",
        width = 20
      )
    normalization_mass_range_entry <-
      tkentry(
        preproc_window,
        textvariable = normalization_mass_range2,
        font = entry_font,
        bg = "white",
        width = 20,
        justify = "center"
    ...
    # Spectral alignment
    spectral_alignment_button <-
      tkbutton(
        preproc_window,
        text = "Align spectra",
        command = spectral_alignment_choice,
        font = button_font,
        bg = "white",
        width = 20
      )
    # Commit preprocessing
    commit_preprocessing_button <-
      tkbutton(
        preproc_window,
        text = "Commit preprocessing",
        command = commit_preprocessing_function,
        font = button_font,
        bg = "white",
        width = 20
      )
  ...
  file_type_export_choice <- function() {
    ...
    # Set the value of the displaying label
    file_type_export_value_label <-
      tklabel(
        window,
        text = file_type_export,
        font = label_font,
        bg = "white",
        width = 20
      )
  ...
  set_file_name <- function() {
    ...
    filename <-
      paste(filename, " (", final_date_time, ")", sep = "")
  ...
  dump_parameters <- function() {
    parameter_vector <-
      c(
        spectra_format_value,
        peak_picking_algorithm_value,
        SNR_value,
        signals_to_take_value,
        peak_deisotoping_enveloping_value,
        low_intensity_peak_removal_threshold_percent_value,
        low_intensity_peak_removal_threshold_method,
        peak_filtering_threshold_percentage_value,
        peak_filtering_mode_value,
        average_replicates_value,
        mass_range_value,
        tof_mode_value,
        tolerance_ppm_value,
        transform_data_value,
    ...
    names(parameter_vector) <-
      c(
        ...
        "Peak deisotoping / enveloping",
        "Low-intensity peak removal threshold percentage",
        "Low-intensity peak removal method",
        "Peak filtering threshold percentage",
        "Peak filtering mode",
        "Average replicates",
        "Mass range",
        "TOF mode",
  ...
  select_samples_function <- function() {
    ...
    if (spectra_input_type == "folder") {
      filepath_import_select <-
        tkmessageBox(title = "Samples",
                     message = "Select the folder for the spectra to be imported.\nIf there are spectral (imzML, txt, csv) files in the folder, each one of them should contain spectra from the same class.\nIf there are subfolders, each one of them should contain spectra from the same class as imzML/txt/csv files",
                     icon = "info")
      ...
      filepath_import_select <-
        tkmessageBox(title = "Samples",
                     message = "Select the file for the spectra to be imported",
                     icon = "info")
      # Filter openable files according to the format
      if (spectra_format == "imzML") {
        filepath_import <-
          tclvalue(tkgetOpenFile(filetypes = "{{imzML files} {.imzML}}"))
      } else if (spectra_format == "txt") {
        filepath_import <-
          tclvalue(tkgetOpenFile(filetypes = "{{TXT files} {.txt}}"))
      } else if (spectra_format == "csv") {
        filepath_import <-
          tclvalue(tkgetOpenFile(filetypes = "{{CSV files} {.csv}}"))
      }
      if (!nchar(filepath_import)) {
  ...
  peak_picking_algorithm_choice <- function() {
    ...
    peak_picking_algorithm_value_label <-
      tklabel(
        window,
        text = peak_picking_algorithm_value,
        font = label_font,
        bg = "white",
        width = 20,
        height = 2
  ...
  peak_deisotoping_enveloping_choice <- function() {
    # Catch the value from the menu
    peak_deisotoping_enveloping <-
      select.list(
        c("Peak Deisotoping", "Peak Enveloping", "None"),
        title = "Peak Deisotoping/Enveloping",
        multiple = FALSE,
        preselect = "Peak Deisotoping"
      )
    # Default
    ...
    peak_deisotoping_enveloping_value_label <-
      tklabel(
        window,
        text = peak_deisotoping_enveloping_value,
        font = label_font,
        bg = "white",
        width = 20
      )
  ...
  average_replicates_choice <- function() {
    ...
    if (average_replicates == "YES" || average_replicates == "") {
      ...
      averaging_method_input <-
        select.list(
          c("Average Spectrum", "Skyline Spectrum", "Median Spectrum"),
          title = "Choose the averaging method",
          preselect = "Average Spectrum"
        )
      # Determine the value
      if (averaging_method_input == "Average Spectrum" ||
          averaging_method_input == "") {
        averaging_method <- "mean"
    ...
    average_replicates_value_label <-
      tklabel(
        window,
        text = average_replicates_value,
        font = label_font,
        bg = "white",
        width = 20,
        height = 2
  ...
  peak_filtering_mode_choice <- function() {
    ...
    peak_filtering_mode <-
      select.list(
        c("whole dataset", "class-wise"),
        title = "Peak filtering mode",
        multiple = FALSE,
        preselect = "whole dataset"
      )
    # Default
    ...
    peak_filtering_mode_value_label <-
      tklabel(
        window,
        text = peak_filtering_mode,
        font = label_font,
        bg = "white",
        width = 20
      )
  ...
  low_intensity_peak_removal_threshold_method_choice <- function() {
    ...
    low_intensity_peak_removal_threshold_method <-
      select.list(
        c("whole", "element-wise"),
        title = "Choose",
        multiple = FALSE,
        preselect = "element-wise"
      )
    # Default
    ...
    low_intensity_peak_removal_threshold_method_value_label <-
      tklabel(
        window,
        text = low_intensity_peak_removal_threshold_method_value,
        font = label_font,
        bg = "white",
        width = 20
      )
  ...
  spectra_format_choice <- function() {
    # Catch the value from the menu
    spectra_format <-
      select.list(c("imzML", "Xmass", "TXT", "CSV", "MSD"),
                  title = "Spectra format",
                  preselect = "Xmass")
    # Default
    ...
    if (spectra_format == "txt") {
      tkmessageBox(title = "TXT format",
                   message = "The TXT file should have two columns: the m/z values and the intensity values, separated by a tab and without any header",
                   icon = "info")
    } else if (spectra_format == "csv") {
      tkmessageBox(title = "CSV format",
                   message = "The CSV file should have two columns: the m/z values and the intensity values, separated by a comma and with a header",
                   icon = "info")
    }
    # Escape the function
    ...
    # Set the value of the displaying label
    spectra_format_value_label <-
      tklabel(
        window,
        text = spectra_format_value,
        font = label_font,
        bg = "white",
        width = 20
      )
  ...
  import_spectra_function <- function() {
    ...
    if (!is.null(filepath_import)) {
      ...
      try({
        ...
        import_progress_bar <-
          tkProgressBar(
            title = "Importing and preprocessing spectra...",
            label = "",
            min = 0,
            max = 1,
            initial = 0,
        ...
        if (spectra_format == "fid" ||
          ...
          spectra <-
            import_spectra(
              filepath = filepath_import,
              spectra_format = spectra_format,
              mass_range = mass_range,
              allow_parallelization = allow_parallelization,
              spectral_names = "name",
              replace_sample_name_field = FALSE,
          ...
          if (length(grep(".imzML", filepath_import, fixed = TRUE)) <= 0) {
            imzml_files <-
              read_spectra_files(filepath_import,
                                 spectra_format = spectra_format,
                                 full_path = TRUE)
          } else {
          ...
          if (!is.null(mass_range)) {
            ...
            if (length(imzml_files) > 0) {
              ...
              for (imzml in 1:length(imzml_files)) {
                ...
                spectra_imzml <-
                  importImzMl(imzml_files[imzml], massRange = mass_range)
                # Preprocessing
                spectra_imzml <-
                  preprocess_spectra(
                    spectra_imzml,
                ...
                if (average_replicates == TRUE) {
                  spectra_imzml <-
                    averageMassSpectra(spectra_imzml, method = averaging_method)
                  # Preprocessing AVG
                  spectra_imzml <-
                    preprocess_spectra(
                      spectra_imzml,
        ...
        } else if ((length(folder_list) == 1 &&
                    folder_list != "") || (length(folder_list) >= 1)) {
      ...
      if (is.null(spectra)) {
        ...
        tkmessageBox(title = "No spectral files",
                     message = "There are no spectral files in the selected folder!\n\nTry to select another folder or another format!",
                     icon = "warning")
      } else {
        tkmessageBox(title = "Import successful",
                     message = "The spectra have been successfully imported and preprocessed",
                     icon = "info")
      }
    } else {
      ### Messagebox
      tkmessageBox(title = "Import not possible",
                   message = "No spectra files or folder have been selected!",
                   icon = "warning")
  ...
  peak_picking_function <- function() {
    ...
    if (!is.null(spectra)) {
      ...
      peak_filtering_threshold_percentage <-
        tclvalue(peak_filtering_threshold_percentage)
      peak_filtering_threshold_percentage <-
        as.numeric(peak_filtering_threshold_percentage)
      peak_filtering_threshold_percentage_value <-
        as.character(peak_filtering_threshold_percentage)
      ## Low intensity threshold
      low_intensity_peak_removal_threshold_percent <-
        tclvalue(low_intensity_peak_removal_threshold_percent)
      low_intensity_peak_removal_threshold_percent <-
        as.numeric(low_intensity_peak_removal_threshold_percent)
      low_intensity_peak_removal_threshold_percent_value <-
        as.character(low_intensity_peak_removal_threshold_percent)
      ##### Peak alignment
      setTkProgressBar(
        peak_picking_progress_bar,
        value = 0.50,
        ...
        if (isMassPeaksList(peaks)) {
          ...
          for (p in 1:length(peaks_class)) {
            class_vector_for_peak_filtering <-
              append(class_vector_for_peak_filtering,
                     peaks_class[[p]]@metaData$file)
          }
          peaks <-
            align_and_filter_peaks(
              ...
              peak_picking_algorithm = peak_picking_algorithm,
              tof_mode = tof_mode,
              peak_filtering_frequency_threshold_percent = peak_filtering_threshold_percentage,
              class_vector_for_peak_filtering = class_vector_for_peak_filtering,
      ...
      low_intensity_peak_removal_threshold_percent_value <<-
        low_intensity_peak_removal_threshold_percent_value
      ### Messagebox
      tkmessageBox(title = "Peak picking successful",
                   message = "The peak picking process has been successfully performed",
                   icon = "info")
    } else if (is.null(spectra)) {
      ### Messagebox
      tkmessageBox(title = "Spectra not imported",
                   message = "The spectra have not been imported yet.\nImport them before performing the peak picking",
                   icon = "warning")
    }
    # Raise the focus on the main window
    tkraise(window)
    ...
    if (!is.null(peaks)) {
      ...
      if (isMassPeaksList(peaks)) {
        for (p in 1:length(peaks)) {
          number_of_signals_vector <-
            append(number_of_signals_vector, length(peaks[[p]]@mass))
        }
        # Compute the mean and the standard deviation
        mean_signal_number <-
          mean(number_of_signals_vector, na.rm = TRUE)
        median_signal_number <-
          median(number_of_signals_vector, na.rm = TRUE)
        sd_signal_number <-
          sd(number_of_signals_vector, na.rm = TRUE)
        cv_signal_number <-
          (sd_signal_number / mean_signal_number) * 100
        ### Messagebox
        # Message
        message_avg_sd <-
          paste(
            "The mean number of signals in the spectral dataset is:",
            mean_signal_number,
        ...
        tkmessageBox(title = "Mean and SD of the number of signals",
                     message = message_avg_sd,
                     icon = "info")
        signals_avg_and_sd_value <-
          paste0(
            "Mean: ",
        ...
        # Message
        message_avg_sd <-
          paste("The number of signals in the spectrum is:",
                number_of_signals)
        tkmessageBox(title = "Number of signals",
                     message = message_avg_sd,
                     icon = "info")
        signals_avg_and_sd_value <-
          paste0("Number of signals: ", number_of_signals)
      }
      signals_avg_and_sd_value <<- signals_avg_and_sd_value
      # Generate a label to see the output
      signals_avg_and_sd_value_label <-
        tklabel(
          window,
          text = signals_avg_and_sd_value,
          font = label_font,
          bg = "white",
          width = 40,
          height = 5
      ...
      tkmessageBox(title = "Something is wrong",
                   message = "Some elements are needed to perform this operation: make sure that the peak picking process has been performed",
                   icon = "warning")
    }
    # Raise the focus on the main window
    tkraise(window)
  ...
  run_peaklist_export_function <- function() {
    ...
    if (!is.null(spectra) && !is.null(peaks)) {
      ...
      if (file_type_export == "csv") {
        ...
        write.csv(
          parameters_matrix,
          file = paste0(filename_subfolder, " - Parameters.", file_type_export),
          row.names = TRUE,
          col.names = TRUE
        )
      } else if (file_type_export == "xlsx" ||
                 file_type_export == "xls") {
        # Save the files (Excel)
        peaklist <- as.data.frame(peaklist)
        # Generate unique row names
        unique_row_names <-
          make.names(rownames(peaklist), unique = TRUE)
      ...
      ### Messagebox
      tkmessageBox(title = "Done!",
                   message = "The peaklist file has been dumped!",
                   icon = "info")
    } else if (is.null(spectra) || is.null(peaks)) {
      ### Messagebox
      tkmessageBox(title = "Something is wrong",
                   message = "Some elements are needed to perform this operation: make sure that the spectra have been imported and the peak picking process has been performed",
                   icon = "warning")
    }
    # Raise the focus on the main window
    tkraise(window)
  ...
  dump_spectra_files_function <- function() {
    ...
    if (!is.null(spectra) && !is.null(peaks)) {
      ...
      spectra_files_subfolder <-
        file.path(output_folder,
                  paste(filename_subfolder, "- Spectral files"))
      ...
      if (isMassSpectrumList(spectra)) {
        if (is.null(names(spectra))) {
          for (s in 1:length(spectra)) {
            spectra_name_vector <-
              append(spectra_name_vector, spectra[[s]]@metaData$file[1])
          }
        } else {
      ...
      if (length(spectra_name_vector) == length(unique(spectra_name_vector))) {
        ...
        spectra_name_vector <-
          make.names(spectra_name_vector, unique = TRUE)
      }
      ### Dump the spectal files
      # Choose the file format
      spectra_output_format <-
        select.list(
          c("MSD", "TXT"),
          preselect = "MSD",
          multiple = FALSE,
          title = "Select the spectra format"
        )
      ...
      if (spectra_output_format == "MSD") {
        if (isMassSpectrumList(spectra)) {
          if (isMassPeaksList(peaks) && length(peaks) == length(spectra)) {
            for (s in 1:length(spectra)) {
              exportMsd(
                spectra[[s]],
                file = paste(spectra_name_vector[s], ".msd", sep = ""),
                force = TRUE,
                peaks = peaks[[s]]
              )
            }
          ...
          if (isMassPeaks(peaks)) {
            exportMsd(
              spectra,
              file = paste(spectra_name_vector, ".msd", sep = ""),
              force = TRUE,
              peaks = peaks
            )
          } else {
        ...
        if (isMassSpectrumList(spectra)) {
          if (isMassPeaksList(peaks) && length(peaks) == length(spectra)) {
            for (s in 1:length(spectra)) {
              spectra_txt <- matrix(0,
                                    ncol = 2,
                                    nrow = length(spectra[[s]]@mass))
              peaks_txt <-
                matrix(0,
                       ncol = 2,
                       nrow = length(peaks[[s]]@mass))
              ...
              write.table(
                spectra_txt,
                file = paste(spectra_name_vector[s], ".txt", sep = ""),
                row.names = FALSE,
                col.names = FALSE
              )
              write.table(
                peaks_txt,
                file = paste(spectra_name_vector[s], " - Peaks.txt", sep = ""),
                row.names = FALSE,
                col.names = FALSE
              )
            }
          ...
          peaks_txt <-
            matrix(0, ncol = 2, nrow = length(peaks@mass))
          ...
          write.table(
            spectra_txt,
            file = paste(spectra_name_vector, ".txt", sep = ""),
            row.names = FALSE,
            col.names = FALSE
          )
          write.table(
            peaks_txt,
            file = paste(spectra_name_vector, " - Peaks.txt", sep = ""),
            row.names = FALSE,
            col.names = FALSE
          )
        }
      ...
      setwd(output_folder)
      ### Messagebox
      tkmessageBox(title = "Spectra files dumped",
                   message = "The spectra files have been succesfully dumped!",
                   icon = "info")
    } else {
      ### Messagebox
      tkmessageBox(title = "Spectra not loaded or Peaks not picked!",
                   message = "No spectra have been imported yet or no peak picking has been performed!",
                   icon = "warning")
    }
    # Raise the focus on the main window
    tkraise(window)
  ...
  select_samples_button <-
    tkbutton(
      window,
      text = "BROWSE\nSPECTRA...",
      command = select_samples_function,
      font = button_font,
      bg = "white",
      width = 20
    )
  ...
  #### Entries
  # Peak picking method
  peak_picking_algorithm_entry <-
    tkbutton(
      window,
      text = "PEAK PICKING\nALGORITHM",
      command = peak_picking_algorithm_choice,
      font = button_font,
      bg = "white",
      width = 20
    )
  # Signals to take
  signals_to_take_label <-
    tklabel(
      window,
      text = "Most intense signals\nto take\n(0 = retain all peaks)",
      font = button_font,
      bg = "white",
      width = 20
    )
  signals_to_take_entry <-
    tkentry(
      window,
      textvariable = signals_to_take,
      font = entry_font,
      bg = "white",
      width = 5,
      justify = "center"
  ...
  # Peaks filtering threshold
  peak_filtering_threshold_percentage_label <-
    tklabel(
      window,
  ...
  peak_filtering_threshold_percentage_entry <-
    tkentry(
      window,
      textvariable = peak_filtering_threshold_percentage,
      font = entry_font,
      bg = "white",
      width = 5,
      justify = "center"
  ...
  peak_filtering_mode_entry <-
    tkbutton(
      window,
      text = "PEAK FILTERING\nMODE",
      command = peak_filtering_mode_choice,
      font = button_font,
      bg = "white",
      width = 20
    )
  # Peaks deisotoping
  peak_deisotoping_entry <-
    tkbutton(
      window,
      text = "PEAK\nDEISOTOPING\nor\nENVELOPING",
      command = peak_deisotoping_enveloping_choice,
      font = button_font,
      bg = "white",
      width = 20
    )
  # Intensity percentage threshold
  low_intensity_peak_removal_threshold_percent_label <-
    tklabel(
      window,
      text = "Low-intensity peak\nremoval\npercentage threshold",
      font = button_font,
      bg = "white",
      width = 20
    )
  low_intensity_peak_removal_threshold_percent_entry <-
    tkentry(
      window,
      textvariable = low_intensity_peak_removal_threshold_percent,
      font = entry_font,
      bg = "white",
      width = 5,
      justify = "center"
    )
  tkinsert(low_intensity_peak_removal_threshold_percent_entry,
           "end",
           "0")
  # Intensity percentage theshold method
  low_intensity_peak_removal_threshold_method_entry <-
    tkbutton(
      window,
      text = "INTENSITY\nTHRESHOLD\nMETHOD",
      command = low_intensity_peak_removal_threshold_method_choice,
      font = button_font,
      bg = "white",
      width = 20
    )
  # File format
  spectra_format_entry <-
    tkbutton(
      window,
      text = "SPECTRA\nFORMAT",
      command = spectra_format_choice,
      font = button_font,
      bg = "white",
      width = 20
    )
  # File type export
  file_type_export_entry <-
    tkbutton(
      window,
      text = "FILE TYPE\nEXPORT",
      command = file_type_export_choice,
      font = button_font,
      bg = "white",
      width = 20
    )
  # Average the replicates
  average_replicates_button <-
    tkbutton(
      window,
      text = "AVERAGE\nREPLICATES",
      command = average_replicates_choice,
      font = button_font,
      bg = "white",
      width = 20
    )
  ...
  # Import the spectra
  import_spectra_button <-
    tkbutton(
      window,
      text = "IMPORT AND\nPREPROCESS\nSPECTRA...",
      command = import_spectra_function,
      font = button_font,
      bg = "white",
      width = 20
    )
  # Import the spectra
  dump_spectra_files_button <-
    tkbutton(
      window,
      text = "DUMP SPECTRA\nFILES...",
      command = dump_spectra_files_function,
      font = button_font,
      bg = "white",
      width = 20
    )
  ...
  peak_picking_button <-
    tkbutton(
      window,
      text = "PEAK\nPICKING...",
      command = peak_picking_function,
      font = button_font,
      bg = "white",
      width = 20
    )
  # Run the Peaklist Export!!
  peaklist_export_button <-
    tkbutton(
      window,
      text = "EXPORT\nPEAKLIST...",
      command = run_peaklist_export_function,
      font = button_font,
      bg = "white",
      width = 20
    )
  # Average number of signals and standard deviation
  signals_avg_and_sd_button <-
    tkbutton(
      window,
      text = "MEAN +/- SD of\nnumber of signals...",
      command = signals_avg_and_sd_function,
      font = button_font,
      bg = "white",
      width = 20
    )
  ...
  spectra_preprocessing_button <-
    tkbutton(
      window,
      text = "SPECTRA\nPREPROCESSING\nPARAMETERS...",
      command = preprocessing_window_function,
      font = button_font,
      bg = "white",
      width = 20
    )
  ...
  peak_filtering_mode_value_label <-
    tklabel(
      window,
      text = peak_filtering_mode_value,
      font = label_font,
      bg = "white",
      width = 20
    )
